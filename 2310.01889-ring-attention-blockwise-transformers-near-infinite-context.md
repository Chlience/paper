...
- [Hao Liu](/authors/hao-liu/): UC Berkeley in the paper; first and corresponding author, email `hao.liu@cs.berkeley.edu`; current public profile states Google DeepMind researcher and incoming CMU machine learning assistant professor. His homepage lists Ring Attention, Blockwise Parallel Transformer, Large World Model and related long-context work; current GitHub `haoliuhl/ringattention` hosts the implementation.
...
- [Pieter Abbeel](/authors/pieter-abbeel/): UC Berkeley professor, Berkeley Robot Learning Lab director and BAIR co-director; OpenReview records Hao Liu's PhD advisor relation with Pieter Abbeel. In this paper, he connects the method to RL, agentic Transformer and long-horizon learning settings.

关系判断：

- 同机构作者群：三位作者在论文署名中均为 UC Berkeley。Hao Liu 与 Pieter Abbeel 存在明确博士生-导师关系；Matei Zaharia 与两位共同形成 Berkeley AI/systems 合作线。
- 跨机构桥接：论文标题页只列 UC Berkeley；作者公开档案显示后续桥接到 Google DeepMind/CMU、Databricks、Amazon FAR 等机构，但这些属于论文发表后的稳定身份信息。
- 与已存档作者重叠：当前没有直接作者重叠。论文致谢感谢 [Tri Dao](/authors/tri-dao/) 对工作的反馈，因此与 FlashAttention 系列存在技术交流线索。
- 与已存档论文的主题或方法关系：与 [2205.14135](/papers/2205.14135-flashattention-io-aware-exact-attention/) 和 [2307.08691](/papers/2307.08691-flashattention-2-parallelism-work-partitioning/) 共享 exact blockwise softmax attention 和 online softmax 语境；与 [1910.02054](/papers/1910.02054-zero-memory-optimizations-trillion-parameter-models/) 共享分布式训练内存问题；与 [2405.17381](/papers/2405.17381-various-lengths-constant-speed-lightning-attention/)、[2606.10650](/papers/2606.10650-dynamic-linear-attention/) 和 [2026-04-24](/papers/2026-04-24-deepseek-v4-million-token-context-intelligence/) 共同构成长上下文系统路线图。
- 需要后续确认：若后续归档 Blockwise Parallel Transformer (`2305.19370`) 或 Large World Model (`2402.08268`)，应把 Hao Liu / Pieter Abbeel 的连续 long-context 研究线补成更完整的 cluster。

作者页决策：

- tracked: Hao Liu、Matei Zaharia、Pieter Abbeel。三者均有高置信主页、学术档案和 X 账号证据；Pieter Abbeel 不记录 GitHub，因为未发现可信个人 GitHub 证据。
...
- 与 `papers-index.md` 新增 cluster：建议新增 Ring Attention / distributed exact attention / context parallelism 节点，连接 FlashAttention family、ZeRO/FSDP、Sarathi 和 long-context architecture/serving 材料。

## Reference Intake Brief

### Target

- Intended target system: 新增论文笔记、作者页、`papers-index.md` long-context systems 图谱。
...
- Proposed form: 新建独立 Markdown 文档；更新作者档案；更新索引 cluster 和跨论文关系。
