...
- [Chao Jin](/authors/chao-jin-pku/): Peking University；equal contribution；与 [2606.04101 UltraEP](/papers/2606.04101-ultraep-rack-scale-moe-load-balancing/) 直接重叠。
- [Ziheng Jiang](/authors/ziheng-jiang/): ByteDance Seed；equal contribution；公开 profile 连接 NSDI 2024 MegaScale 和 EuroSys 2026 MegaScale-MoE。
...
- [Size Zheng](/authors/size-zheng/): ByteDance Seed；公开 homepage / Scholar 连接系统与 kernel 方向。EuroSys accepted-papers 页面检索结果中出现 `Xuegui Zheng`，arXiv v3、TeX source 和 ACM reference format 写作 `Size Zheng`，本笔记以 arXiv / TeX 为主。
- [Yanghua Peng](/authors/yanghua-peng/): ByteDance Seed；与 [2409.19256 HybridFlow](/papers/2409.19256-hybridflow-rlhf-framework/) 直接重叠。
- [Haibin Lin](/authors/haibin-lin/): ByteDance Seed；与 HybridFlow / DAPO / verl 系统线直接重叠。
- Xuanzhe Liu: Peking University；acknowledgements 说明 Chao Jin、Xuanzhe Liu、Xin Jin also with Key Laboratory of High Confidence Software Technologies, Peking University, Ministry of Education。
- [Xin Jin](/authors/xin-jin-pku/): Peking University；corresponding author；PKU systems senior author 线索。
- Xin Liu: ByteDance Seed；corresponding author；与 [2503.14476 DAPO](/papers/2503.14476-dapo-long-cot-rl-system/) 中 ByteDance Seed `Xin Liu` 存在同名同机构系统线索，仍需个人主页进一步确认。

关系判断：

- 同机构作者群：ByteDance Seed 是核心工程组织，覆盖 Ziheng Jiang、Zhihao Bai、Zheng Zhong、Juncai Liu、Xiang Li、Ningxin Zheng、Xi Wang、Cong Xie、Qi Huang、Wen Heng、Yiyuan Ma、Wenlei Bao、Size Zheng、Yanghua Peng、Haibin Lin、Xin Liu；Peking University 覆盖 Chao Jin、Xuanzhe Liu、Xin Jin。
- 跨机构桥接：Chao Jin 和 Ziheng Jiang 为 equal contribution，连接 PKU 与 ByteDance Seed；Xin Jin 和 Xin Liu 为 corresponding authors，连接学术系统组与产业训练基础设施；Chao Jin、Xuanzhe Liu、Xin Jin 还连接 PKU 高可信软件技术重点实验室。
- 与已存档作者重叠：[Chao Jin](/authors/chao-jin-pku/) 与 [2606.04101 UltraEP](/papers/2606.04101-ultraep-rack-scale-moe-load-balancing/) 重叠；[Haibin Lin](/authors/haibin-lin/) 与 [2409.19256 HybridFlow](/papers/2409.19256-hybridflow-rlhf-framework/) 和 [2503.14476 DAPO](/papers/2503.14476-dapo-long-cot-rl-system/) 重叠；[Yanghua Peng](/authors/yanghua-peng/) 与 HybridFlow 重叠；Xin Liu 与 DAPO 存在同名同机构线索。
- 与已存档论文的主题或方法关系：直接连接 [2309.14509 DeepSpeed Ulysses](/papers/2309.14509-deepspeed-ulysses-long-sequence-training/) 的 SP attention、[1910.02054 ZeRO](/papers/1910.02054-zero-memory-optimizations-trillion-parameter-models/) 的 data-parallel state sharding、[2412.19437 DeepSeek-V3](/papers/2412.19437-deepseek-v3-technical-report/) 的 DeepEP / DualPipe 对照、[2606.04101 UltraEP](/papers/2606.04101-ultraep-rack-scale-moe-load-balancing/) 的后续 MoE expert-side load balancing。
- 作者 profile pass：Grok broad search 因 403 spending-limit 阻塞；已用 web 搜索交叉验证 Chao Jin、Ziheng Jiang、Xin Jin、Size Zheng、Haibin Lin 的 homepage / Scholar / X 或机构来源，并更新 `authors.json`。Zhihao Bai 等 ByteDance Seed 作者暂不建档，原因是本地尚未发现跨论文重复或高置信个人主页。
- 需要后续确认：Xin Liu 是否与 DAPO 中 Xin Liu 为同一人；EuroSys 页面中 `Xuegui Zheng` 与 arXiv `Size Zheng` 的 metadata 差异；MegaScale-MoE 是否会公开训练系统代码或更多 kernel 实现。
...
| Grok broad author search | 本地 Grok wrapper 返回 403 spending-limit；改用 web search / homepage / Scholar / X 手动交叉验证。 |
