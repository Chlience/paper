...
- Rulin Shao: University of Washington；equal contribution；corresponding email list 包含 `rulins`。
- Shuyue Stella Li: University of Washington；equal contribution；corresponding email list 包含 `stelli`。
- Rui Xin: University of Washington；equal contribution；corresponding email list 包含 `rx31`。
- Scott Geng: University of Washington；equal contribution；corresponding email list 包含 `sgeng`。
...
- 同机构作者群：University of Washington 是核心作者群，覆盖前七位作者、Yulia Tsvetkov、Luke Zettlemoyer，以及带 UW/AI2 双机构身份的 Ranjay Krishna、Hannaneh Hajishirzi、Pang Wei Koh。
- 跨机构桥接：Nathan Lambert 代表 AI2；Ranjay Krishna、Hannaneh Hajishirzi、Pang Wei Koh 连接 UW 与 AI2；Sewon Min 连接 UC Berkeley。
- 项目组织：Rulin Shao、Shuyue Stella Li、Rui Xin、Scott Geng 为 equal contribution，并且四人的 UW 邮箱出现在 correspondence 字段。作者群呈现 UW 主导、AI2 深度参与、Berkeley 补充合作的结构。
- 与已存档作者重叠：未发现与当前 `papers-index.md` 已存档论文作者的直接重叠。
- 与已存档论文的主题或方法关系：与 `2504.13837` 共同分析 RLVR 是否真正扩展 reasoning boundary；与 `2503.14476`、`2501.12948` 共享 GRPO / RLVR / verifiable reward 主线；与 `2501.09620`、`2403.03185` 共享 spurious reward / proxy reward / reward hacking 诊断语言；与 `2605.30290` 共享 verifier/reward feedback 质量问题；与 `2606.00135` 共享 harness、prompt、model family 对 RL 结论的影响；与 `2503.11926` 共享训练信号改变可观察 reasoning 行为的安全视角。
- 需要后续确认：论文使用 ICML 模板和 v2 arXiv 版本，当前笔记未确认正式会议接收状态。后续若作者发布更大规模非 Qwen 复验、更新代码或补充训练日志，应同步更新结论边界。
...
- Existing related assets: `papers-index.md`；`2504.13837-rlvr-reasoning-boundary-base-model.md`；`2503.14476-dapo-long-cot-rl-system.md`；`2501.12948-deepseek-r1-rl-reasoning.md`；`2403.03185-correlated-proxies-reward-hacking.md`。
