...
- [Peiyi Wang](/authors/peiyi-wang/): Peking University；标题页脚注说明主要贡献完成于 DeepSeek-AI internship；后续 DeepSeek-R1 core contributor。
- [Lei Li](/authors/lei-li-hku/): The University of Hong Kong；主页和公开档案显示研究方向覆盖 LLM、machine translation、reasoning 和 agent。
- [Zhihong Shao](/authors/zhihong-shao/): Tsinghua University / DeepSeek AI；主页列出 Math-Shepherd、DeepSeekMath、DeepSeek-R1、DeepSeek-Prover 和 ToRA。
- [Runxin Xu](/authors/runxin-xu/): DeepSeek-AI；论文署名为 R.X. Xu；个人主页显示 DeepSeek-R1、DeepSeek-V 系列、DeepSeekMath 和 MoE 方向。
...
- [Yifei Li](/authors/yifei-li-osu/): The Ohio State University；此前有 Peking University 背景，后续研究方向覆盖 LLM agents 和 scientific coding agents。
- [Deli Chen](/authors/deli-chen/): DeepSeek-AI；主页列出 DeepSeek-R1、DeepSeek-V 系列、DeepSeek-Coder 和 step-by-step verification。
- [Yu Wu](/authors/yu-wu-deepseek/): DeepSeek-AI；主页显示其团队推动 GRPO 和 DeepSeek-R1-Zero 方向。
- [Zhifang Sui](/authors/zhifang-sui/): Peking University；计算语言学方向 senior author，OpenReview 档案与 PKU 学生网络关系强。

关系判断：

- 同机构作者群：Peking University / PKU NLP 线包括 Peiyi Wang、Damai Dai、Zhifang Sui，并通过 Peiyi Wang 的 DeepSeek internship 连接 DeepSeek-AI；DeepSeek-AI 作者群包括 Runxin Xu、Deli Chen、Yu Wu，并与 Zhihong Shao、Damai Dai、Peiyi Wang 有直接研究交集。
- 跨机构桥接：Lei Li 连接 HKU NLP 线；Yifei Li 连接 OSU / PKU 背景；Zhihong Shao 连接 Tsinghua 和 DeepSeek reasoning 线；Peiyi Wang 是 PKU 到 DeepSeek 的关键桥接。
- 与已存档作者重叠：与 [2501.12948 DeepSeek-R1](/papers/2501.12948-deepseek-r1-rl-reasoning/) 存在多位核心人物重叠，尤其是 Peiyi Wang、Runxin Xu、Zhihong Shao、Yu Wu；与 DeepSeek-V 系列和 DeepSeekMath 系谱也存在作者网络关系。
- 与已存档论文的主题或方法关系：直接承接 [2305.20050 Let's Verify Step by Step](/papers/2305.20050-lets-verify-step-by-step-process-supervision/) 的 process reward / step-level verifier 目标，把人工 first-error label 成本替换为自动 completion-based pseudo-label；与 [2605.30290 STV](/papers/2605.30290-self-trained-verification/) 共享 verifier-guided self-improvement；与 [2605.25507 SRPO](/papers/2605.25507-credit-assignment-resets-language-model-reasoning/) 同属把 outcome correctness 局部化到 reasoning step 的 credit assignment 方向。
- 需要后续确认：公开资料未发现完整 step-by-step PPO 训练代码；Hugging Face model card 明确说明内部 step-wise PPO 代码无法开源，复现只能依赖 released checkpoints、数据和论文超参。
...
- 与 `papers-index.md` 中已有论文的作者关系：与 [2501.12948 DeepSeek-R1](/papers/2501.12948-deepseek-r1-rl-reasoning/) 存在 Peiyi Wang、Runxin Xu、Zhihong Shao、Yu Wu 等作者网络重叠；与 DeepSeek-Math、DeepSeek-Prover、DeepSeek-V 系列有机构和研究线联系。
