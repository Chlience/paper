...
对一次 update `W -> W - Z`，训练 loss decrease 可用二阶 Taylor approximation 表示：
...
- `<G, Z>` 是 first-order decrease，表示 update 与 gradient 的对齐收益。
- `1/2 * <Z, H[Z]>` 是 curvature penalty，表示沿这个方向走一步被局部曲率抵消掉的收益。
...
```text
Z_t^Adam = eta_t * M'_t / (sqrt(V'_t) + eps)
```
...
- diagonal blocks `H_ll`：within-layer curvature。
- off-diagonal blocks `H_ll'`：cross-layer interactions。
...
- 指标：realized loss decrease、predicted loss decrease、first-order decrease `<G, Z>`、curvature penalty `1/2 <Z, H[Z]>`。
...
- 指标：`||Z||_F^2`、NDS、curvature penalty，Adam-to-Muon ratios。
...
- 设置：Zipf-PCFG corpus，imbalance levels `s = 0, 0.5, 1`；9M NanoGPT，4 layers、4 heads、dimension 256；训练 10,000 steps。
...
- 指标：`S_F^within`、`S_F^cross`、within-layer fraction。
...
- 结果：Theorem 5.5 证明，在这些假设下，对任意 finite horizon `T >= 1`，Muon 的 average NDS 小于 GD；若 curvature ratio `rho = w_H / w_L` 足够大并满足 `rho + 1 > 1/alpha > 1 + sigma_H/sigma_L`，Muon 在同样步数后获得更低 local quadratic loss。
...
- 设置：使用 Zipf-PCFG + Muon 训练后的 checkpoint，对 attention matrices `W_Q, W_K, W_V, W_O` 计算 dense Hessian；每个 block Hessian 为 `65,536 x 65,536`。
