...
#### 5.1 执行顺序视角：一次 forward kernel 如何跑
...
FA2 仍然按 block 处理 sequence：

$$
Q=[Q_1,\ldots,Q_{T_r}],\qquad K=[K_1,\ldots,K_{T_c}],\qquad V=[V_1,\ldots,V_{T_c}]
$$

其中 $Q_i\in\mathbb{R}^{B_r\times d}$，$K_j,V_j\in\mathbb{R}^{B_c\times d}$，$T_r=\lceil N/B_r\rceil$，$T_c=\lceil N/B_c\rceil$。每个 thread block 负责一个 $Q_i$ row block。它把 $Q_i$ 从 HBM load 到 SRAM / registers，然后顺序扫描所有 $K_j,V_j$ blocks。
...
#### 5.3 Backward：用重算换 HBM traffic，并重新组织梯度数据流
...
#### 5.4 沿 sequence dimension 增加 parallelism
...
#### 5.5 改变 warp-level work partitioning：从 sliced-K 到 sliced-Q

Thread block 内部通常有 4 或 8 个 warps。FA1 和 FA2 的关键差异在于这些 warps 如何分工。
...
#### 5.6 Causal mask、MQA/GQA、block size 与 head dimension
