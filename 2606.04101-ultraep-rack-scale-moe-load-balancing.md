...
- 每 rank redundant slot 数 `N_slot`。
...
```text
T_solve_rep^fwd
+ max(T_reroute^fwd, T_w_distr^fwd)
+ T_tok_a2a^fwd
+ T_moe^fwd
```
...
```text
T_moe ∝ max_r sum_e u_{e,r}
```

token all-to-all 同时受最忙 sender 和 receiver 影响：

```text
T_tok_a2a ∝ max_r max(sum_e lambda_{r,e}, sum_e u_{e,r})
```

weight distribution 由承载最多 hot main experts fan-out 的 rank 主导：

```text
T_w_distr ∝ max_r sum_{e in E_r} (|H(e)| - 1)
```

约束包括：main placement 不变、每 rank redundant slots 有限、同一 logical expert 不能在同一 rank 出现多次、新 replica 至少承载 `u_min` token、replica 引入的 backward communication 需要尽量被计算 overlap 隐藏。
...
3. 在 average load 和 max load 之间 binary search 最小可行 threshold `tau`。
4. 对某个 `tau`，定义每个 rank 的 excess 和 slack：

```text
exc_r(tau) = max(ell_r - tau, 0)
slk_r(tau) = max(tau - ell_r, 0)
```

5. 贪心 feasibility oracle 访问 overloaded ranks 和 hot main experts，把 expert load 迁移到 slack 最大且满足 slot/no-duplicate/u_min 的 rank。
6. 如果所有 excess 都被 drain，说明当前 `tau` feasible，继续搜索更小 threshold。
