...
Error 规则在实现前先对全库运行。缺少 `Workflow version: v2` 的历史条目使用兼容规则，检查器通过章节别名，以及 Source 中的外部 URL 或有效站内来源链接处理旧格式，避免散落文件名白名单。带有 v2 标记的条目执行完整 Source、时间、结果定位、审稿分类和接纳决策规则。历史缺口进入 advisory；未来批量迁移完成后再单独提交严格化改动。
