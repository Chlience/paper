...
`论文脉络` 是单篇笔记最重要的分析正文。验收时先检查它是否给出首要贡献与辅助贡献的全景，再检查方法是否按照实际执行顺序或理论论证的依赖顺序展开。方法包含多个关键阶段、角色或策略变体时，需要先给端到端概览并消解同名对象的层级歧义。每个关键环节需要说明输入或初始条件、核心操作、处理的局部问题、传递对象、输出或训练信号、设计理由、直接证据，以及失败条件或成立边界。

详细方法直接使用 `### 5.`、`### 6.` 等连续编号的同级标题，首节承担贡献全景和端到端概览，省略只用于包裹这些章节的“核心贡献与方法逻辑”标题。结论链条等后续章节使用下一个未占用的整数。方法内容本身只有一个实质章节时，可以保留单个 `### 5. 方法…`。
...
方法详细度以读者能够复述完整执行链或论证链为准，不设统一字数。自动检查只验证显式贡献—方法结构和输入、传递、输出、理由、证据与边界等信号是否存在，内容完整性、顺序合理性、公式解释和例子准确性仍由人工语义门禁判断。
...
上述质量依赖人工语义判断。自动化只检查规范文档之间的契约一致性、选定真实 canary 的结构与证据定位，以及全库既有 CLI 是否保持零错误；不对存量笔记强制三级标题、固定字段或关键词。
