...
阶段详解从 `### 6.` 开始，按照实际执行顺序或理论论证的依赖顺序连续编号。至少展开首要机制；紧密耦合阶段可以合并说明，只有省略会妨碍复述首要机制、直接证据或成立边界的阶段才单独成节。详解说明输入或初始条件、核心操作、处理的局部问题、传递对象、输出或训练信号、设计理由、直接证据，以及失败条件或成立边界。后文增加关键角色、分支、传递对象或训练信号时同步回写总览。结论链条使用方法末节之后的下一个整数。
...
方法详细度按两层阅读结果验收，不设统一字数：只阅读第 5 节时，读者能够复述首要贡献、完整执行链或论证链、关键对象及其角色差异，以及最终输出、训练信号或论证结论；阅读必要详解后，读者能够进一步复述首要机制的接口、信号方向、设计理由、直接证据和成立边界。自动检查只接受顶层第 5 节，并剔除 HTML 注释后按两种模式验证：多阶段总览检查首要贡献、辅助贡献、输入、顺序、操作、传递、输出和作用，单阶段总览检查首要贡献、输入、操作、输出、理由、直接证据和成立边界；多阶段笔记至少保留一个阶段详解。内容完整性、总览与详解的语义对应、公式解释和例子准确性仍由人工门禁判断。
...
`主要启发` 是单篇笔记中稳定认识更新的唯一归档位置。候选通过认识变化、区分性证据和迁移预测筛选时，`archive-core` 至多构建一条与首要贡献对齐的首要启发；没有独立认识更新时省略本节。辅助启发只在用户明确要求时增加；v2.1 格式继续接受历史笔记和增强任务中证据充分的多条启发。

内部构建依次检查：可归因的原有判断、材料中的认识触发点、作者执行的关键转换、连接转换与结果的作用机制、能够排除更简单解释的区分性证据、由此形成的新判断、保留条件与逻辑关系的抽象关系、在目标对象中的重新实例化与可证伪预测，以及论文事实、作者解释、用户判断、本地分析和待验证外推的来源与边界。

公开正文以 `### N. <可独立成立的新判断>` 命名，并写成二至三个自然段：先用论文专属证据解释认识变化与机制，再给出抽象关系、目标实例化和迁移预测，最后在需要时说明成立条件、失败情形、替代解释和来源边界。方法摘要、结果复述、通用提醒、未披露项、复验清单、宽泛类比和无区分性证据的机制猜测分别回流到 `论文脉络`、`关键实验/定理`、`局限`、代码与复现或 `跨论文关系`。

新契约自 2026-08-05 10:40 起适用于新建或实质更新的 v2.1 笔记。自动检查只验证编号标题、自然段和直接证据定位；旧笔记达到事实纠错、一句话结论真值变化或用户明确维护这三类回写条件时迁移。认识变化、机制充分性、迁移预测、来源归因和失效边界继续由人工门禁判断。

后续交流按陈述功能直接写入当前笔记的 `论文脉络`、`关键实验/定理`、`局限`、`跨论文关系` 或 `主要启发`，不再新增公开的 `本地讨论补充`。旧笔记达到回写条件时迁移存量章节并去重，不执行一次性全库重写。

上述质量依赖人工语义判断。自动化检查规范文档之间的契约一致性、第 5 节标题与结构信号、阶段详解信号、主要启发的新契约结构、选定真实 canary 的结构与证据定位，以及全库既有 CLI 是否保持零错误；冻结基线内尚未迁移的存量笔记继续产生提示。自动检查不能替代总览与详解的语义对应、认识变化、机制充分性、迁移预测、来源归因和结论边界判断。
...
- `Key figure decision: include`：图片提供文字或表格无法紧凑替代的机制、数据路径、训练信号、主结论或失败边界，或者用户明确要求图片；正文至少嵌入一张本地图片。
- `Key figure decision: omit`：文字或表格能够完整表达核心机制和证据，或图片受版权、安全与材料形态限制；用 `Key figure rationale` 说明原因。

图片数量由不可替代的信息价值决定。装饰图和可由文字或紧凑表格完整表达的少量数字使用 `omit`；综述中的 taxonomy、领域地图和概念层级图只有在能显著降低阅读成本时使用 `include`。
...
常规归档完成后运行一次 `check:source`；失败时只重跑受影响检查和最终一次 `check:source`。production build、生成页面检查和部署由 GitHub Actions 完成；用户明确要求本地复现完整 CI 或排查构建差异时运行 `npm run check:all`。`dist/` 与 `src/generated/` 不进入 commit。
