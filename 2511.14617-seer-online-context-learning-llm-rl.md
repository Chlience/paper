...
- Mingxing Zhang: Tsinghua University; corresponding author, `zhang_mingxing@mail.tsinghua.edu.cn`.

关系判断：

- 同机构作者群：Moonshot AI 作者群覆盖 Weiran He、Weixiao Huang、Yangkun Zhang、Yikai Zhao、Bo Pang、Xinran Xu，并通过 Ruoyu Qin 连接 Tsinghua；Tsinghua University 作者群覆盖 Ruoyu Qin、Yingdi Shan、Yongwei Wu、Mingxing Zhang。
- 跨机构桥接：Ruoyu Qin 具有 Moonshot AI 和 Tsinghua University 双重署名，是产业训练系统与高校系统研究之间的桥接作者；Mingxing Zhang 为通讯作者，承担 Tsinghua 侧沟通角色。
...
- 需要后续确认：是否会公开 Seer 代码、Moonshot 内部 production workloads 细节、Mooncake global KVCache pool 的可复现配置。
...
### 3. Grouped CST 如何生成当前请求的 draft tokens
