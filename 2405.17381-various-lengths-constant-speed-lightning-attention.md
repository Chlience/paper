...
- Yiran Zhong: OpenNLPLab, Shanghai AI Lab；corresponding author / arXiv submitter。

关系判断：

- 同机构作者群：Weigao Sun、Dong Li、Xuyang Shen、Weixuan Sun、Yiran Zhong 同属 OpenNLPLab / Shanghai AI Lab，是本文核心研究与工程作者群。
- 跨机构桥接：Zhen Qin 连接 TapTap 与 OpenNLPLab / Shanghai AI Lab 作者群。
- 系列关系：`2401.04658` Lightning Attention-2 和本文作者列表完全相同。`2401.04658` 是 technical report，本文是 ICML 2024 版本，增加了 TransNormerLLM 架构、更多实验和更完整的论文叙事。
- 与已存档作者重叠：与 [2506.13585](/papers/2506.13585-minimax-m1-cispo-lightning-attention/) 存在 Dong Li、Xuyang Shen、Yiran Zhong 同名线索。由于 MiniMax-M1 title page 只以 MiniMax 统一署名，appendix 没有逐作者机构，本笔记只记录同名线索，暂不合并身份。
- 与已存档论文的主题或方法关系：本文是 [2506.13585](/papers/2506.13585-minimax-m1-cispo-lightning-attention/) 中 Lightning Attention / TransNormer 系统线的上游来源；与 [2606.06453](/papers/2606.06453-vortex-sparse-attention-serving/)、[2511.02749](/papers/2511.02749-span-queries-cache-attention-locality/)、[2405.19888](/papers/2405.19888-parrot-semantic-variable-llm-serving/) 共同服务 long-context / long-output 计算效率图谱；与 [2605.14220](/papers/2605.14220-training-inference-mismatch-llm-rl/) 在长上下文架构进入 RL 后的 sampler/trainer 数值一致性问题上相关。
- 需要后续确认：MiniMax-M1 contributor list 中同名作者是否与本文作者完全相同；OpenNLPLab / Shanghai AI Lab 后续 TransNormerLLM 与 MiniMax 系列之间是否有公开项目迁移说明；Lightning Attention kernel 在现代 vLLM/SGLang/训练框架中的实际维护状态。
...
- 与 [2606.06453](/papers/2606.06453-vortex-sparse-attention-serving/)、[2511.02749](/papers/2511.02749-span-queries-cache-attention-locality/)、[2405.19888](/papers/2405.19888-parrot-semantic-variable-llm-serving/) 的关系：本文从模型架构和 kernel 层降低长序列成本；这些 serving 论文从 sparse attention、KV locality、application DAG 角度降低真实 workload 成本。
...
- 新增后应更新的索引 cluster：新增 “Lightning Attention 与 TransNormerLLM” cluster，并连接到 MiniMax-M1、Long-context Serving、Training-Inference Mismatch 和 RL systems compute accounting。
...
- Existing related assets: `papers-index.md`；已存档 MiniMax-M1 笔记 [2506.13585](/papers/2506.13585-minimax-m1-cispo-lightning-attention/)。
- Proposed form: 新建独立 Markdown 文档，并更新 `papers-index.md` 和 MiniMax-M1 相关来源说明。
