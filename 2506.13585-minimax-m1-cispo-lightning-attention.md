...
- `sg` (stop-gradient，停止梯度)：前向计算保留数值，反向传播时把该项当作常数。CISPO 中的 $\mathrm{sg}(\hat r)$ 表示 clipped IS weight 只作为梯度系数使用，梯度流向后面的 $\log \pi_\theta$，不再通过 $\hat r(\theta)$ 或内部的 ratio / clip branch 反传。
...
论文实验中没有实际设置下界，只调 $\epsilon_{\mathrm{high}}^{\mathrm{IS}}$。原文只说明 $\mathrm{sg}$ 表示 stop-gradient，但这个记号在 CISPO 里承担关键语义：$\hat r_{i,t}(\theta)$ 先用 current / old policy ratio 算出数值，再经过 clip 得到权重；加上 $\mathrm{sg}$ 后，反向传播不会穿过 ratio 或 clip，只会把这个权重乘到 $\nabla_\theta \log \pi_\theta(o_{i,t}\mid q,o_{i,<t})$ 上。也就是说，CISPO 优化的是 score-function gradient，clipped IS weight 只决定该 token 梯度系数。

如果去掉 $\mathrm{sg}$，目标中的 $\hat r(\theta)\log \pi_\theta$ 会额外产生穿过 $\hat r(\theta)$ 的梯度项；在未 clipped 区间，这相当于同时优化 ratio 本身和 logprob，偏离 REINFORCE / IS-weighted policy gradient 的解释；在 clipped 区间，clip branch 的导数虽然为 0，$\log \pi_\theta$ 仍会提供梯度，语义会随 ratio 区间切换而变得混杂。$\mathrm{sg}$ 把问题固定成更清晰的形式：所有 token 都通过 $\log \pi_\theta$ 更新，权重最多被 $\epsilon_{\mathrm{high}}^{\mathrm{IS}}$ 限制。它牺牲一部分无偏性，换取更低方差和更连续的 token-level learning signal。
...
$M_{i,t}$ 在这里表示 RL objective 里的 token-level gradient switch，需要和 attention mask / padding mask 区分开。$M_{i,t}=1$ 表示第 $i$ 条 response 的第 $t$ 个 token 参与 policy gradient 更新；$M_{i,t}=0$ 表示这个 token 在当前 objective 中不贡献 $\log \pi_\theta$ 梯度。它把“是否保留 token 梯度”和“保留后给多大权重”拆开：前者由 $M_{i,t}$ 控制，后者由 $\mathrm{sg}(\hat r_{i,t})\hat A_{i,t}$ 控制。

PPO trust region 可被视作一种隐式 token mask：当 positive-advantage token 的 ratio 太高，或 negative-advantage token 的 ratio 太低时，相关 token 梯度会被丢掉，相当于在这些越界方向上让 $M_{i,t}=0$。CISPO 的默认取向是 $M_{i,t}=1$，保留所有 token 的 score-function gradient，再通过 clipped weight 控制幅度。

从实现路径看，CISPO 相对朴素 GRPO/DAPO 改了两个关键位置：

1. 朴素 clipped surrogate 直接对 $r_{i,t}\hat A_{i,t}$ 和 clipped branch 取较小项，边界外可能变成常数；CISPO 通过 $\mathrm{sg}$ 把 $\hat r_{i,t}$ detach 后作为权重，梯度仍来自 $\log \pi_\theta(o_{i,t}\mid q,o_{i,<t})$。
...
- TIS 在 TIM/rollout correction 语境中通常处理 rollout behavior 与 trainer old policy 的 mismatch。$r_{\mathrm{corr},t}=\pi_{\mathrm{old}}^{\mathrm{train}}(a_t\mid s_t)/\pi_{\mathrm{old}}^{\mathrm{rollout}}(a_t\mid s_t)$ 只是 raw correction ratio；`Truncated` 发生在把它变成受限权重时，例如 $w_t^{\mathrm{TIS}}=\min(r_{\mathrm{corr},t},\tau_{\mathrm{tok}})$，或再配合 sequence-level rejection / clipping。目标是把训练解释校回真实采样分布，同时限制 rollout/trainer mismatch outlier。
