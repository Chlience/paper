...
- [Zhewei Kang](/authors/zhewei-kang/): 论文署名 UC Berkeley；arXiv submitter；代码仓库位于 GitHub `backprop07/VIMPO`；OpenReview profile 显示其教育经历包含 University of Hong Kong 与 UC Berkeley，并列出 Self-Certainty、Intuitor 和 VIMPO 相关论文线。
- [Aosong Feng](/authors/aosong-feng/): 论文署名 Yale University；主页显示其当前为 AWS Bedrock Applied Scientist，PhD from Yale University，研究 long-horizon human-AI interaction、agentic systems 和 LLM routing；GitHub `asFeng`。
- [Sergey Levine](/authors/sergey-levine/): UC Berkeley EECS associate professor；RAIL Lab 负责人之一，长期研究 reinforcement learning、robotics、decision making；本篇中提供 RL / policy optimization 背景。
- [Dawn Song](/authors/dawn-song/): UC Berkeley Computer Science professor；BAIR / RDI / CHAI 相关研究线，研究 AI safety and security、Agentic AI、deep learning、decentralization technology、security and privacy。
- [Xuandong Zhao](/authors/xuandong-zhao/): UC Berkeley postdoctoral researcher，BAIR / RDI；个人主页显示与 Dawn Song 合作，研究 machine learning、NLP、AI safety，重点包括 scalable reinforcement learning、self-improvement、frontier AI reliability；主页列出 VIMPO、Intuitor、Self-Certainty 等同组工作。

关系判断：

- 同机构作者群：UC Berkeley 是核心节点，包含 Zhewei Kang、Sergey Levine、Dawn Song、Xuandong Zhao；Aosong Feng 以 Yale University 署名加入，并通过 Intuitor / agentic AI 研究线与该组连续合作。
- 跨机构桥接：Aosong Feng 连接 Yale / AWS Bedrock 与 UC Berkeley VIMPO/Intuitor 线；Xuandong Zhao 连接 UC Berkeley BAIR / RDI、UCSB PhD 背景和 Dawn Song 研究组；Sergey Levine 带来 Berkeley RAIL / RL 方法论背景，Dawn Song 带来 Berkeley AI safety/security 背景。
- 与已存档作者重叠：本次新增前 `authors.json` 未记录这 5 位作者。Sergey Levine 和 Dawn Song 与本地 Berkeley RL / AI safety cluster 存在机构关系；Dawn Song 与 [2403.03185](/papers/2403.03185-correlated-proxies-reward-hacking/) 的 Berkeley AI safety 语境相近，但无作者重叠。
- 与已存档论文的主题或方法关系：和 [2503.14476](/papers/2503.14476-dapo-long-cot-rl-system/)、[2026-06-16 verl](/papers/2026-06-16-verl-rl-optimization-algorithms/)、[2505.22617](/papers/2505.22617-entropy-mechanism-rl-reasoning-language-models/)、[2504.13837](/papers/2504.13837-rlvr-reasoning-boundary-base-model/)、[2506.10947](/papers/2506.10947-spurious-rewards-rethinking-rlvr/) 共享 RLVR / GRPO / token-level credit assignment / reasoning gain 来源问题。
- 需要后续确认：VIMPO 是否进入公开会议审稿；代码仓库是否后续释放完整数据、训练脚本日志和多 seed 复现实验；Intuitor 和 VIMPO 是否形成 Berkeley 内部连续 RLIF/RLVR 方法线。
- 作者页决策：5 位作者均 tracked。理由是 VIMPO、Intuitor、Self-Certainty 构成连续 Berkeley reasoning RL 线，且 Sergey Levine、Dawn Song、Xuandong Zhao 具有强稳定公开 profile。
...
| 全量 X 深挖 | Xuandong、Sergey、Dawn 的 X 可由主页/GitHub直接交叉验证；Zhewei、Aosong 未找到高置信个人 X，暂记 not-found。 |
