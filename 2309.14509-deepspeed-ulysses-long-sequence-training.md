...
| 顺序 | 模块 / activation | TP 每卡形状 | TP 冗余在哪里 | Ulysses 每卡形状 | Ulysses 怎么省 |
| --- | --- | --- | --- | --- | --- |
| 1 | layer input / residual stream | `[B, N, H]` | 每个 TP rank 都有完整 sequence 和完整 residual hidden，形成 $P$ 份重复 | `[B, N/P, H]` | sequence 常驻状态直接除以 $P$ |
| 2 | pre-attn LayerNorm input / output / stats | `[B, N, H]`，stats `[B, N]` | LayerNorm 作用在 hidden 维，但每张卡仍持有完整 tokens | `[B, N/P, H]`，stats `[B, N/P]` | LayerNorm 只处理本地 tokens |
| 3 | QKV projection input | `[B, N, H]` | QKV linear 的输入 activation 在 TP ranks 上 full-sequence 复制 | `[B, N/P, H]` | 每张卡只为本地 tokens 算 Q/K/V |
| 4 | QKV projection output | `[B, N, 3H/P]` | hidden/head 已切，基本没有 hidden 冗余；但仍覆盖完整 $N$ | `[B, N/P, 3H]`，随后 all-to-all 成 `[B, N, 3H/P]` | 元素数同为 $3BNH/P$，Ulysses 改 layout，不复制全量 |
| 5 | attention 前布局 | `[B, N, heads/P, d]` | TP 的 head-parallel attention 核心：full sequence + subset heads | all-to-all 后同样是 `[B, N, heads/P, d]` | attention 内临时变成 TP-like head split |
| 6 | QK / softmax / AV 中间量 | 若 materialize: `[B, heads/P, N, N]` | dense attention 的二次项，TP 和 Ulysses 都绕不开 | 同 TP | Ulysses 对 dense attention core 没有额外降低；通常依赖 FlashAttention 避免保存完整 attention matrix |
| 7 | attention context before output merge | `[B, N, H/P]` | head shard，无 hidden 冗余，但是 full sequence | `[B, N, H/P]` | 和 TP 类似 |
| 8 | attention context 回到层主布局 | TP 通常接 row-parallel output projection / all-reduce，回到 `[B, N, H]` | attention 后又恢复 full-sequence full-hidden replicated activation | 第二次 all-to-all 回到 `[B, N/P, H]` | 离开 attention 后立刻回到 sequence shard |
| 9 | post-attn dropout / residual | `[B, N, H]` | 每个 TP rank 都保存完整 tokens 的 residual 结果 | `[B, N/P, H]` | per-token activation 省 $P$ 倍 |
| 10 | pre-MLP LayerNorm | `[B, N, H]` | full sequence 复制 | `[B, N/P, H]` | 只处理本地 sequence |
| 11 | MLP up/gate projection input | `[B, N, H]` | 输入 full-sequence 复制 | `[B, N/P, H]` | 输入 activation 省 $P$ 倍 |
| 12 | MLP intermediate / GeLU / SwiGLU | `[B, N, αH/P]` | hidden 已切，但是 full sequence | `[B, N/P, αH]` | 元素数同为 $\alpha BNH/P$；TP 切 hidden，Ulysses 切 sequence |
| 13 | MLP down projection output | row-parallel 后常回到 `[B, N, H]` | full-sequence full-hidden 再次复制 | `[B, N/P, H]` | 后续层继续 sequence shard |
| 14 | layer output / next residual | `[B, N, H]` | 每层边界 full activation 复制 | `[B, N/P, H]` | 层边界 activation 省 $P$ 倍 |
