...
First-Archived-At: 2026-06-18 22:13
Updated-At: 2026-06-18 22:13
...
- Authors: [Samyam Rajbhandari](/authors/samyam-rajbhandari/), Jeff Rasley, Olatunji Ruwase, [Yuxiong He](/authors/yuxiong-he/)
...
- [Samyam Rajbhandari](/authors/samyam-rajbhandari/): Microsoft；标题页邮箱为 `samyamr@microsoft.com`。论文脚注标记其与 Jeff Rasley 为 equal contributors。
...
- [Yuxiong He](/authors/yuxiong-he/): Microsoft；标题页邮箱为 `yuxhe@microsoft.com`。

关系判断：

- 同机构作者群：四位作者均使用 Microsoft 邮箱，论文实现和开源计划落在 Microsoft DeepSpeed 项目中。
- 跨机构桥接：标题页未展示跨机构作者；本篇更像 Microsoft 内部系统团队产出。
- 与已存档作者重叠：[Samyam Rajbhandari](/authors/samyam-rajbhandari/) 和 [Yuxiong He](/authors/yuxiong-he/) 与 [2309.14509](/papers/2309.14509-deepspeed-ulysses-long-sequence-training/) 直接重叠，形成 DeepSpeed ZeRO / Ulysses 连续系统线。
...
这里通信的对象是 gradients 和更新后的 fp16 parameter shards，optimizer states 本身不在每个 step 做全量通信。rank $r$ 长期保存自己负责参数片对应的 fp32 master weights、momentum、variance；backward 后通过 gradient reduce-scatter 拿到这片参数的 averaged gradient shard，本地完成 Adam update，再把更新后的 parameter shard all-gather 给其他 ranks。

更具体地说，每个参数 shard 可以看成一个有状态的 optimizer instance。它拥有该 shard 的 master weights、$m$、$v$，每步只接收对应的 averaged gradient shard，然后独立推进本地 Adam 状态。其他 ranks 只需要最终更新后的 fp16 parameter shard 来参与下一轮 forward/backward。
...
在 $P_{os}$ 中，通信调度可以复用这个分解：gradient reduce-scatter 需要 $\Psi$，更新后的 parameter shard all-gather 需要 $\Psi$，所以：

$$
C_{P_{os}} = 2\Psi.
$$

如果朴素地先做完整 gradient all-reduce，再额外 all-gather 更新后的参数，通信会变成 $3\Psi$。论文的 $2\Psi$ 分析对应的是优化后的调度：省掉标准 all-reduce 的 gradient all-gather 半段，把这部分通信预算用于 all-gather updated parameters。
...
1. $P_{os}$：optimizer states 分片。每个 rank 保存自己负责参数片的 fp32 master weights、momentum、variance。backward 后通过 reduce-scatter 拿到自己负责参数片的 averaged gradient shard，然后本地更新这一片参数。更新完成后 all-gather 最新参数，让所有 rank 重新拥有完整 fp16 parameters，供下一轮 forward/backward 使用。optimizer states 长期留在 owner rank，本身不做 step 级全量通信。
...
4. Optimizer update：rank $r$ 只用自己负责参数片的 reduced gradients 更新本地 fp32 master weights、momentum、variance。
5. Parameter sync：各 rank 把更新后的 parameter shards all-gather 给所有 rank，使下一步 forward 重新拥有完整 fp16 parameters。

这样 ZeRO-1 的 step 级通信仍是 $2\Psi$：一次 gradient reduce-scatter 加一次 updated-parameter all-gather。朴素实现若先做完整 gradient all-reduce，再同步更新后的参数，会额外多出一次 parameter all-gather。
...
2. Backward：梯度按 bucket 产生。bucket ready 后立即 reduce-scatter，rank $r$ 只收到自己负责参数片的 averaged gradient shard。
3. Optimizer update：rank $r$ 用本地 gradient shard 更新本地 optimizer-state shard 和 master-weight shard。
...
$$
\text{w/o ZeRO: local grad bucket}
\xrightarrow{\mathrm{all\text{-}reduce}}
\text{replicated averaged grad bucket}
\xrightarrow{\mathrm{local\ Adam}}
\text{replicated updated params}.
$$

$$
\text{w/ ZeRO-2: local grad bucket}
\xrightarrow{\mathrm{reduce\text{-}scatter}}
\text{averaged grad shard}
\xrightarrow{\mathrm{sharded\ Adam}}
\text{updated param shard}
\xrightarrow{\mathrm{all\text{-}gather}}
\text{replicated updated params}.
$$
...
6. Optimizer update：rank $r$ 更新自己的 parameter shard 和 optimizer-state shard。step 结束后仍保持 shard-only 状态，下一步 forward 再按 layer gather。
...
- 与 [2309.14509 DeepSpeed Ulysses](/papers/2309.14509-deepspeed-ulysses-long-sequence-training/)：作者上有 [Samyam Rajbhandari](/authors/samyam-rajbhandari/) 和 [Yuxiong He](/authors/yuxiong-he/) 直接重叠。ZeRO 处理 model states redundancy；Ulysses 处理 sequence activation 和 attention QKV/context layout。两者共同说明 DeepSpeed 系统线如何把 aggregate GPU memory 和 interconnect 用到不同训练状态上。
