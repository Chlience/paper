...
Policy optimization 使用 online mirror descent 风格更新。直观上，它先从当前 policy 采样多条 response，根据 reward 得到相对好坏，再把新 policy 往高 reward response 方向移动，同时用 reference policy 约束每步更新不要离当前模型太远。它省去 value network 和 `PRM (Process Reward Model，过程奖励模型)`，也不依赖 MCTS 式显式树搜索；和 [GRPO](/papers/2501.12948-deepseek-r1-rl-reasoning/) 相似，它依赖同一 prompt 下多条采样的 reward baseline 降低方差。差别在于 k1.5 把 long context、采样策略和 length penalty 视为同一个 policy optimization recipe 的组成部分。
