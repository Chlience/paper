...
- Nick Mitchell: IBM Research, New York, USA；corresponding author，邮箱 `nickm@us.ibm.com`；arXiv submission history 显示本文由 Nick Mitchell 提交。
...
- 同机构作者群：Paul Castro、Nick Mitchell、Mudhakar Srivatsa 属 IBM Research New York；Nathan Ordonez、Thomas Parnell 属 IBM Research Zurich；Antoni Viros i Martin 属 IBM Research Massachusetts。
- 跨机构桥接：本文是 IBM Research 内部跨 New York、Zurich、Massachusetts 三地协作；Nick Mitchell 同时承担通讯作者和 arXiv 提交者角色，关系判断上是主要联络节点。
- 项目组织：作者群集中在 IBM Research，主题覆盖 LLM serving、KV cache、vLLM runtime 和 ReRoPE / positional encoding 相关系统实现。
- 与已存档作者重叠：未发现与当前存档论文存在直接作者重叠。
- 与已存档论文的主题或方法关系：与 [2405.19888](/papers/2405.19888-parrot-semantic-variable-llm-serving/) 关系最强，二者都要求应用把结构暴露给 serving backend；Parrot 暴露 Semantic Variable / DAG / performance objective，本文暴露 Span Query expression tree 和 commutativity constraints。与 [2606.06453](/papers/2606.06453-vortex-sparse-attention-serving/) 同属 serving / attention 系统方向；Vortex 让 sparse attention algorithm 可编程和可部署，本文让 cache locality 与 attention locality 可以由上层 query structure 驱动。与 [2409.19256](/papers/2409.19256-hybridflow-rlhf-framework/) 都把 dataflow 交给系统调度器，只是 HybridFlow 面向 RLHF/RLVR training，本文面向 inference serving。与 [2606.00135](/papers/2606.00135-agentic-tool-calling-rl-training/) 在 agentic / tool-use workflow 上相连，后者的长轨迹和多轮工具返回可以成为 Span Query 需要优化的 serving workload。与 [2025-09-10](/papers/2025-09-10-defeating-nondeterminism-llm-inference/) 和 [2605.14220](/papers/2605.14220-training-inference-mismatch-llm-rl/) 的关系在于：Span Query 的重排、批处理和 KV 重定位若进入 RL rollout 或评测，需要同步审计 backend determinism 与 rollout/trainer consistency。
- 需要后续确认：是否有开源 vLLM patch、CIDRA implementation、REST front-end 或 MLSys camera-ready 版本；特殊 token 对真实任务准确率的影响是否有更完整实验。
...
- Existing related assets: `papers-index.md`；相关已存档论文包括 [2405.19888](/papers/2405.19888-parrot-semantic-variable-llm-serving/)、[2606.06453](/papers/2606.06453-vortex-sparse-attention-serving/)、[2409.19256](/papers/2409.19256-hybridflow-rlhf-framework/)、[2606.00135](/papers/2606.00135-agentic-tool-calling-rl-training/)。
- Proposed form: 新建独立 Markdown 文档，并更新索引中的 LLM serving / Span Query cluster。
