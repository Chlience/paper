...
- Yang Yue（乐洋）: LeapLab, Tsinghua University；equal contribution；project lead；贡献声明说明其领导项目，最早发现 RL 后 pass@$k$ 被 base model 超过的现象，提出研究想法，设计并部分执行实验，主要负责论文写作。
- Zhiqi Chen: LeapLab, Tsinghua University；equal contribution；贡献声明说明其执行大量实验，包括跨模型/benchmark 的 pass@$k$ 评估和 perplexity analysis，并参与讨论、作图和稿件审阅。
- Rui Lu: LeapLab, Tsinghua University；贡献声明说明其参与想法启发、项目概念化、story writing 和 AI reasoning trajectory 人工检查。
- Andrew Zhao: LeapLab, Tsinghua University；贡献声明说明其参与实验设计讨论，提出 perplexity-based analysis，并参与早期 RL training code 实现。
- Zhaokai Wang: Shanghai Jiao Tong University；贡献声明说明其参与 RLVR 对 reasoning boundary 影响的讨论、写作、校对和整体审阅。
- Yang Yue（乐阳）: LeapLab, Tsinghua University；贡献声明说明其参与 visual reasoning model 训练、讨论、校对和图表润色。论文特别说明第一作者 Yang Yue（乐洋）和第六作者 Yang Yue（乐阳）英文名相同、中文名不同。
- Shiji Song: LeapLab, Tsinghua University；贡献声明说明其监督研究并协助论文写作。
- Gao Huang: LeapLab, Tsinghua University；通讯作者标注；贡献声明说明其监督研究并协助论文写作。

关系判断：

- 同机构作者群：LeapLab, Tsinghua University 是核心作者群，包含两位 Yang Yue、Zhiqi Chen、Rui Lu、Andrew Zhao、Shiji Song 和 Gao Huang；Zhaokai Wang 来自 Shanghai Jiao Tong University。
- 跨机构桥接：Zhaokai Wang 构成 Tsinghua LeapLab 与 Shanghai Jiao Tong University 之间的协作桥接。
- 项目组织：Yang Yue（乐洋）是 project lead；Yang Yue（乐洋）与 Zhiqi Chen 为 equal contribution；Gao Huang 和 Shiji Song 承担监督角色；邮件信息包含 `le-y22`、`zq-chen23` 和 `gaohuang`。
- 与已存档作者重叠：未发现与当前 `papers-index.md` 已存档论文作者的直接重叠。存在 Tsinghua 机构网络关联：`2503.14476` DAPO 涉及 AIR Tsinghua / SIA-Lab of Tsinghua AIR and ByteDance Seed，但未发现作者重叠。
- 与已存档论文的主题或方法关系：与 `2501.12948` DeepSeek-R1、`2503.14476` DAPO 共享 RLVR / long-CoT reasoning RL 主线；与 `2605.30290` STV 都关心 verifier / reward feedback 是否真正扩展能力；与 `2606.00135` tool-calling RL 共享 RL training efficiency 与 exploration 问题；与 `2503.11926` CoT monitoring 共享“训练信号改变可观察分布”的视角；与 `2403.03185` correlated proxies 共享 reference/base distribution 与 policy shift 分析语言。
- 需要后续确认：v5 是 2025-11-24 的 camera-ready 版本，后续若作者发布更大规模 RLVR compute、multi-turn agentic RL 或 dense process reward 复验，需要更新本文结论边界。
