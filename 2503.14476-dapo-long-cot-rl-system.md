...
- 原始 GRPO 的 reduction 是 sample-level：先对单条 response 内的 token loss 取平均，再对同一 prompt group 的 responses 取平均。形式上近似 `1/G * sum_i (1/|o_i| * sum_t loss_{i,t})`。这样每条 response 权重相同，长 response 内每个 token 的单点权重更低。
- DAPO 改为 token-level reduction：把 group 内所有 response token 放在同一个 token 池里统一平均，形式上近似 `1/sum_i |o_i| * sum_i sum_t loss_{i,t}`。这样每个 token 的权重一致，长 response 因 token 更多，对整体梯度贡献也更大。
...
- 论文设置 `L_max = 20,480`，`L_cache = 4,096`，所以安全区间到 `L_max - L_cache = 16,384`。长度不超过 16,384 不罚；16,384 到 20,480 之间线性扣分；超过 20,480 扣到 `-1`。
- 公式可以写成：
...
- 最终 reward 近似是 `R_total = R_correctness + R_length`。这样模型仍能通过正确性 reward 学 reasoning，同时逐步学会在长度预算内完成答案。
