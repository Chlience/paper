...
- Kimi Team / Moonshot AI: 论文以 Kimi Team 署名，arXiv 作者栏显示 Tongtong Bai and 324 other authors。Hugging Face model card 由 `moonshotai/Kimi-K2.5` 发布，模型 license 标记为 `modified-mit`。
...
- 同机构作者群：Kimi Team 是主组织单元，实际研究覆盖模型架构、视觉编码器、预训练、SFT、RL、Agent Swarm、部署和评测等多个工程小组。
- 跨机构桥接：论文正文和 model card 都指向 Moonshot AI / Kimi 生态；appendix 的 HKU 脚注提供额外外部合作线索。由于 v1 没有逐作者 affiliation 表，本笔记只记录可直接从标题页、附录和 model card 验证的关系。
- 与已存档作者重叠：和 [2511.14617](/papers/2511.14617-seer-online-context-learning-llm-rl/) 有 Moonshot AI 作者重叠，说明 K2.5 的 agentic RL / serving / rollout 工程和 Seer 的 synchronous RL rollout 系统位于同一公司生态内。
- 与已存档论文的主题或方法关系：强连接 [2506.13585](/papers/2506.13585-minimax-m1-cispo-lightning-attention/)、[2602.15763](/papers/2602.15763-glm-5-agentic-engineering/)、[2026-06-17](/papers/2026-06-17-slime-rl-scaling-framework/)、[2026-06-16](/papers/2026-06-16-verl-rl-optimization-algorithms/)、[2511.14617](/papers/2511.14617-seer-online-context-learning-llm-rl/)、[2606.12370](/papers/2606.12370-bebop-mtp-rejection-sampling-rl-training/)、[2503.14476](/papers/2503.14476-dapo-long-cot-rl-system/)。
- 作者 profile pass：团队型 325 作者报告；本轮复用已建档的 Moonshot/Seer/Kimi 作者，并记录 Tao Yu 的 HKU 脚注线索。未批量创建完整作者页，原因是 v1 缺少逐人 affiliation / contribution，自动建档会降低作者图质量。
- 需要后续确认：Kimi K2.5 训练数据比例、Agent Swarm 任务集合、sub-agent checkpoint 选择策略、reward model 训练细节、开源 checkpoint 与报告中评测 checkpoint 是否完全一致。
...
- 指标与结果摘录：HLE-Full 30.1，HLE-Full with tools 50.2，AIME 2025 96.1，GPQA-Diamond 87.6，MMLU-Pro 87.1，SWE-Bench Verified 76.8，Terminal Bench 2.0 50.8，MMMU-Pro 78.5，VideoMMMU 86.6，OSWorld-Verified 63.3。
- 解读：K2.5 的定位是 broad agentic model，强项不只在静态文本 benchmark，也包括工具、视频、网页和 OS 操作。

### 实验设置与 baseline 审计

- 模型与初始化：Kimi K2 text MoE backbone + MoonViT-3D；1T total / 32B activated；256K context。
...
- 系统配置：DEP、Unified Agentic RL Environment、LLM Gateway、rollout logprob recording、partial rollout、100,000 concurrent agent tasks；具体集群和 wall-clock 成本未完整公开。
