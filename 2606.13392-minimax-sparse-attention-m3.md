...
- [Xunhao Lai](/authors/xunhao-lai/): MiniMax; Peking University. TeX 注释将其放在 `algorithm & training infra` author group。主页显示其研究方向为 long context models、efficient attention、training / inference efficiency，并列出 FlexPrefill。
- [Weiqi Xu](/authors/weiqi-xu-minimax/): MiniMax. Hugging Face profile `xiaoxiaoOWO` 显示其为 MiniMax Sparse Attention paper author。
- [Yufeng Yang](/authors/yufeng-yang-minimax/): MiniMax. 同名出现在 MiniMax-01、MiniMax-M1、MiniMax-M2 和 MSA 作者/贡献者列表中。
- [Qiaorui Chen](/authors/qiaorui-chen/): NVIDIA. 论文中唯一非 MiniMax 主署名机构作者；公开搜索另有 NVIDIA Shanghai / ORCID 线索，但未找到个人主页。
- [Yang Xu](/authors/yang-xu-minimax/): MiniMax; Zhejiang University. 主页显示浙江大学本科生、MiniMax intern，从事 LLM pretraining 和 model architecture design；GitHub 与主页互链，X 为 `@YangXu_09`。
- [Lunbin Zeng](/authors/lunbin-zeng/): MiniMax; Huazhong University of Science and Technology. GitHub/Scholar 显示 HUST 硕士生，研究 LLM / MLLM；与 MiniMax-M1 直接重叠。
- [Xiaolong Li](/authors/xiaolong-li-minimax/): MiniMax; Zhejiang University.
- [Haohai Sun](/authors/haohai-sun-minimax/): MiniMax. Together AI event materials 称其为 MiniMax Research Scientist；MiniMax 官方 M3 社交帖标注 `@HaohaiSun`；同名出现在 MiniMax-01/M1/M2。
- [Haichao Zhu](/authors/haichao-zhu-minimax/): MiniMax. 与 MiniMax-M1/M2 直接重叠。
- [Vito Zhang](/authors/vito-zhang-minimax/): MiniMax; Peking University.
- [Jinkai Hu](/authors/jinkai-hu-minimax/), [Jiayao Li](/authors/jiayao-li-minimax/), [Rui Gao](/authors/rui-gao-minimax/), [Zekun Li](/authors/zekun-li-minimax/), [Songquan Zhu](/authors/songquan-zhu-minimax/), [Jingkai Zhou](/authors/jingkai-zhou/): TeX 注释将这组作者放在 `inference infra` author group；Rui Gao 另署 Nanjing University，Jingkai Zhou 另署 Hangzhou Dianzi University。
- [Pengyu Zhao](/authors/pengyu-zhao-minimax/): MiniMax. TeX 注释将其标为 `lead`；公开资料显示其为 MiniMax LLM Lead / Head of Research，X `@zpysky1125` 长期发布 MiniMax M 系列相关内容。

关系判断：

- 同机构作者群：MiniMax 是核心机构，17 位作者中 16 位署名 MiniMax；Qiaorui Chen 署名 NVIDIA。
- 跨机构桥接：Peking University 通过 Xunhao Lai、Vito Zhang 连接 MiniMax；Zhejiang University 通过 Yang Xu、Xiaolong Li 连接 MiniMax；HUST 通过 Lunbin Zeng 连接 MiniMax；Nanjing University 通过 Rui Gao 连接 MiniMax；Hangzhou Dianzi University 通过 Jingkai Zhou 连接 MiniMax；NVIDIA 通过 Qiaorui Chen 连接 MiniMax 的 kernel / infra 线。
- 与已存档作者重叠：本文新增作者档案。与 [2506.13585 MiniMax-M1](/papers/2506.13585-minimax-m1-cispo-lightning-attention/) 的 contributor list 存在 Yufeng Yang、Lunbin Zeng、Haohai Sun、Haichao Zhu、Pengyu Zhao、Songquan Zhu 等直接同名且同 MiniMax 语境重叠；与 `MiniMax-01` 和 `MiniMax-M2` 也存在多名重叠。由于 M1 是团队型 alphabetical contributor list，关系判断以 MiniMax 组织连续产出为主，个人贡献不从排序推断。
- 与已存档论文的主题或方法关系：本文直接连接 [2506.13585 MiniMax-M1](/papers/2506.13585-minimax-m1-cispo-lightning-attention/)、[2405.17381 Lightning Attention](/papers/2405.17381-various-lengths-constant-speed-lightning-attention/)、[2606.10650 Dynamic Linear Attention](/papers/2606.10650-dynamic-linear-attention/)、[2026-04-24 DeepSeek-V4](/papers/2026-04-24-deepseek-v4-million-token-context-intelligence/)、[2606.06453 Vortex](/papers/2606.06453-vortex-sparse-attention-serving/)、[2511.02749 Span Query](/papers/2511.02749-span-queries-cache-attention-locality/)、[2205.14135 FlashAttention](/papers/2205.14135-flashattention-io-aware-exact-attention/) 和 [2307.08691 FlashAttention-2](/papers/2307.08691-flashattention-2-parallelism-work-partitioning/)。
- 作者 profile pass：已尝试按 `author-x-account-search-sop.md` 使用 Grok broad search，但本地 Grok 返回 `403 Forbidden: SuperGrok Heavy subscription required`；随后改用网页检索、GitHub、主页、Scholar、Hugging Face、X 搜索结果和官方/机构页面交叉验证。已将 17 位作者写入 `authors.json`：Xunhao Lai、Yang Xu、Lunbin Zeng 有较强 homepage/GitHub/Scholar 证据；Pengyu Zhao 与 Haohai Sun 有高置信 X / MiniMax 组织证据；其余作者以 arXiv/MiniMax/HF paper evidence 建最小档案并标注 `xConfidence: "not-found"`。
- 需要后续确认：MSA full training kernels 是否会公开到 H800 / SM90 路线；当前 GitHub `MiniMax-AI/MSA` README 主要描述 `fmha_sm100` / NVIDIA SM100 依赖，而论文效率实验报告 H800；M3 428B 模型完整训练 recipe、数据、RL / SFT / multimodal curriculum 和 MSA 在 full 428B 模型上的 ablation 仍待更完整报告。
...
- 作者 profile pass 已覆盖公开主页、GitHub、Scholar/HF/X 候选；Grok 默认路径不可用，X 判断只记录高置信候选。
...
3. 稀疏 attention 的理论 FLOPs 只有在 kernel 处理好 Top-k、反向索引、hot block load balancing、query gather 和 split softmax 之后才有生产价值。于是论文把方法和 kernel 设计合写，而不是只给 attention 公式。
...
- Exp-free Top-k：Top-k 只关心 score ordering，因此不需要先做 softmax；直接在 raw scores 上选择。
- Specialized Top-k kernel：采用 `B_k=128`、`k=16`，warp lanes 分片扫描候选 blocks，用 shared-memory min-heap 和 shuffle merge。论文报告在 H800 上相对 `torch.topk` 和 TileLang radix-select 更快，部署 setting `128K / 1024 blocks / k=16` 下为 `779us`，相对 `torch.topk 3970us` 是 `5.1x`，相对 TileLang `2864us` 是 `3.7x`。
...
- RL / 训练配置：本文不是 RL paper；没有 post-training / RL objective 审计。M3 的 coding / cowork 能力来自 release surface，训练闭环未公开到可复查级别。
...
- 评测协议：覆盖一般推理、数学、代码、图像、视频、RULER、HELMET、agent task PPL；多数结果是 benchmark point estimate，缺少 seed / error bar。
- 统计报告：未见多 seed、置信区间或显著性检验。
- Baseline 强度：
  - 是否 tuned：Full GQA、MSA-PT、MSA-CPT 是同架构 family、matched budget 对比，baseline 强度较好；但 MSA 的 hyperparameter search 细节未完全公开。
  - 是否 compute-matched：主质量实验按 3T token budget 对齐；效率实验按同 head configuration 对齐。
  - 是否 implementation-matched：同一实验 family，可信度较高；外部复现依赖开源 kernel 和训练栈成熟度。
  - 是否覆盖强替代方案：相关 work 对比讨论 NSA、MoBA、DSA、FlashSparse 等，但主实验没有在同一 109B 训练设置下重跑这些替代 sparse attention。
  - 是否存在弱化风险：Full GQA 是最自然 baseline，但缺少与 NSA / DSA / MoBA 在同规模、同硬件、同数据下的 head-to-head。
...
- 作者关系中部分个人 profile 证据强，部分只来自 arXiv 和 MiniMax author lists；已在 `authors.json` 中区分。
...
6. 作者 profile pass 对 MiniMax 内部作者只能建立最小档案；除少数有主页/GitHub/X 的作者外，贡献角色仍主要来自 TeX 注释和组织线索。

## 跨论文关系

- 与 `papers-index.md` 中已有论文的作者关系：本文与 [2506.13585 MiniMax-M1](/papers/2506.13585-minimax-m1-cispo-lightning-attention/) 存在 MiniMax 组织和多名 contributor 同名重叠，尤其是 Yufeng Yang、Lunbin Zeng、Haohai Sun、Haichao Zhu、Pengyu Zhao、Songquan Zhu。M1 的作者列表按字母序，本文 TeX 中额外有 `algorithm & training infra`、`inference infra`、`lead` 注释，因此本文提供了比 M1 更细的 MiniMax 内部技术角色线索。
...
- 新增后应更新的索引 cluster：新增或扩展 MiniMax cluster 为 “MiniMax-M1/M3、Lightning Attention、MSA 与长上下文架构效率”；在 long-context / sparse attention / serving locality 关系段落中加入 `2606.13392`。
...
- Existing related assets: `papers-index.md`；[2506.13585 MiniMax-M1](/papers/2506.13585-minimax-m1-cispo-lightning-attention/)；[2405.17381 Lightning Attention](/papers/2405.17381-various-lengths-constant-speed-lightning-attention/)；[2606.10650 DLA](/papers/2606.10650-dynamic-linear-attention/)；[2026-04-24 DeepSeek-V4](/papers/2026-04-24-deepseek-v4-million-token-context-intelligence/)；[2606.06453 Vortex](/papers/2606.06453-vortex-sparse-attention-serving/)。
- Proposed form: 新建独立 Markdown 文档 `2606.13392-minimax-sparse-attention-m3.md`，更新 `papers-index.md` 和 `authors.json`。
...
| Grok author broad search | 本地 Grok wrapper 返回 `403 Forbidden: SuperGrok Heavy subscription required`，已改用网页检索。 |
