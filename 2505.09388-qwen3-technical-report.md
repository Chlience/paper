...
- 与已存档作者重叠：已为 [An Yang](/authors/an-yang-qwen/)、[Binyuan Hui](/authors/binyuan-hui/)、[Bowen Yu](/authors/bowen-yu-qwen/)、[Bo Zheng](/authors/bo-zheng-qwen/)、[Rui Men](/authors/rui-men/)、[Jianwei Zhang](/authors/jianwei-zhang-qwen/)、[Jianxin Yang](/authors/jianxin-yang-qwen/)、[Fei Huang](/authors/fei-huang-qwen/)、[Junyang Lin](/authors/junyang-lin/)、[Dayiheng Liu](/authors/dayiheng-liu/)、[Jingren Zhou](/authors/jingren-zhou-qwen/) 建档；这些作者与 [2606.12370 Bebop](/papers/2606.12370-bebop-mtp-rejection-sampling-rl-training/) 存在直接重叠。
- 与已存档论文的主题或方法关系：Qwen3 是 Bebop、STV、OTB、VIMPO、Qwen-family RLVR 审计的基础模型族；也连接 thinking / non-thinking mode 控制。
- 作者 profile pass：团队报告不批量新增全体作者；已为跨 Qwen2.5 / Qwen3 / Bebop 反复出现的核心作者建立 profile。
...
预训练分三段。S1 general stage 使用超过 30T tokens，sequence length 4K，负责通用知识、多语、代码和基础能力。S2 reasoning stage 加入约 5T STEM、code、reasoning 高质量 tokens，把模型分布推向可推理任务。S3 long-context stage 使用数千亿 long-context tokens、32K sequence，并继续沿用 [YaRN](https://arxiv.org/abs/2309.00071) / `DCA (Dual Chunk Attention，双块注意力)` 做 4x 推理外推；DCA 的机制边界可参考 [Qwen2.5-1M](https://qwenlm.github.io/blog/qwen2.5-1m/) 线索，即通过重映射相对位置降低 RoPE 在超长距离上的外推压力。
