...
- 作者 profile pass：这是 185 人团队型模型报告；本轮保留 GLM-5 Team、tech leads、advisors 与 core contributors 作为关系节点，不批量为完整作者列表建档，原因是论文缺少逐人机构和贡献映射，低置信逐人建档会污染作者图。已经在 slime / GLM-5.2 / IndexCache 相关条目中出现的 Xin Lv、Chengxing Xie、Zilin Zhu、Yushi Bai、Qian Dong、Ting Jiang、Zhengxiao Du、Aohan Zeng、Jie Tang、Juanzi Li 等，应按跨论文证据逐步建档或补强。
...
- 模型设置：GLM-5 是 744B total / 40B active MoE，3 dense layers、75 MoE layers、256 experts、每 token 8 routed experts + 1 shared expert，目标上下文 200K。
...
- RL 设置：post-training 包含 SFT、Reasoning RL、Agentic RL、General RL 和 on-policy cross-stage distillation；Reasoning RL 使用 GRPO + IcePop 风格 mismatch correction；Agentic RL 使用 slime 异步 rollout、TITO、direct double-sided IS、stale sample dropping、environment failure filtering 和 DP-aware routing。
...
- 索引状态：`papers-index.md` 已包含 “GLM-5 / GLM-5.2、Agentic Engineering 与 Long-Horizon Coding Agents” cluster，并连接 DeepSeek-V4、TIM/VeXact、HybridFlow/slime、tool-use RL、MiniMax-M1/CISPO、DAPO、Vortex/Parrot/Span Query 和 reward hacking。
