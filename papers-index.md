...
- Paper: `2001.08361`
- Institutions: Johns Hopkins University, OpenAI。
- Internal relation: OpenAI 为核心研究组织；Jared Kaplan 连接 Johns Hopkins University 与 OpenAI；Jared Kaplan 与 Sam McCandlish 共同主导研究；Tom Brown、Rewon Child、Scott Gray、Alec Radford 支撑 optimized Transformer implementation；Benjamin Chess、Jeffrey Wu、Alec Radford 支撑 WebText2 text datasets；Dario Amodei 提供全程 guidance。
- Theme relation: neural language model scaling laws, WebText2, decoder-only Transformer, non-embedding parameters, power-law loss scaling, model/data/compute allocation, critical batch size, compute-efficient training, early stopping, sample efficiency, pretraining compute accounting。
