...
| DEEPEP-2026-06-19 | DeepEP V2 / main: Expert Parallel Communication Library | [2026-06-19-deepep-expert-parallel-communication.md](/papers/2026-06-19-deepep-expert-parallel-communication/) | DeepEP, expert parallelism, MoE dispatch/combine, NCCL Gin, ElasticBuffer, FP8 dispatch, RDMA, NVLink, low-SM communication kernels | DeepSeek-AI infrastructure contributors; tracked profiles include [Chenggang Zhao](/authors/chenggang-zhao/), [Shangyan Zhou](/authors/shangyan-zhou/), [Chengqi Deng](/authors/chengqi-deng/), [Zhean Xu](/authors/zhean-xu/); recurring team credits include Liyue Zhang, Yuxuan Liu, Kuai Yu, Jiashi Li, Liang Zhao |
| DEEPGEMM-2026-06-19 | DeepGEMM main: Tensor Core Kernel Library and Mega MoE | [2026-06-19-deepgemm-tensor-core-kernels.md](/papers/2026-06-19-deepgemm-tensor-core-kernels/) | DeepGEMM, FP8/FP4/BF16 GEMM, grouped/masked MoE GEMM, Mega MoE, MQA indexer, HyperConnection, JIT, SM90/SM100 | DeepSeek-AI kernel contributors; tracked profiles include [Chenggang Zhao](/authors/chenggang-zhao/), [Zhean Xu](/authors/zhean-xu/), [Chenhao Xu](/authors/chenhao-xu/), [Shengyu Liu](/authors/shengyu-liu/); recurring team credits include Liang Zhao, Jiashi Li, Anyi Xu, Kexing Zhou, Kuai Yu |
...
### Cluster AC2: DeepSeek Kernel Infrastructure、DeepEP / DeepGEMM 与 Mega MoE

- Material: `DEEPEP-2026-06-19`, `DEEPGEMM-2026-06-19`
- Organization: DeepSeek-AI。
- Internal relation: 两个仓库共享 [Chenggang Zhao](/authors/chenggang-zhao/)、[Zhean Xu](/authors/zhean-xu/)、Kuai Yu、Jiashi Li、Liang Zhao 等作者线索；DeepEP 额外包含 [Shangyan Zhou](/authors/shangyan-zhou/)、Liyue Zhang、[Chengqi Deng](/authors/chengqi-deng/)、Yuxuan Liu；DeepGEMM 额外包含 [Chenhao Xu](/authors/chenhao-xu/)、Anyi Xu、[Shengyu Liu](/authors/shengyu-liu/)、Kexing Zhou。关系判断上，这是 DeepSeek-AI MoE communication 与 tensor-core kernel 两条 infra 线的交汇。
- Theme relation: DeepEP, DeepGEMM, MoE expert parallelism, dispatch/combine, NCCL Gin, ElasticBuffer, FP8 dispatch, FP8/FP4/BF16 GEMM, grouped/masked GEMM, V3.2 MQA indexer, HyperConnection, Mega MoE, communication-computation overlap, SM90/SM100 JIT kernels, DeepSeek-V4 kernel stack。
...
- `DEEPEP-2026-06-19` 新增 DeepEP expert-parallel communication library 节点。它把 DeepSeek MoE 系统里的 dispatch/combine、FP8 token transfer、NCCL Gin、ElasticBuffer、低 SM 占用通信和 RDMA/NVLink topology 作为独立基础层记录，补齐 V4/UltraEP/slime 中此前只被引用的底层通信库。
- `DEEPGEMM-2026-06-19` 新增 DeepGEMM tensor-core kernel library 节点。它把 FP8/FP4/BF16 GEMM、MoE grouped/masked GEMM、V3.2 MQA indexer、HyperConnection 和 Mega MoE 放入同一套 JIT CUDA kernel 语境，补齐 DeepSeek-V4 中 MegaMoE、deterministic kernels、FP4 QAT 和 indexer 的公开仓库入口。
- `DEEPEP-2026-06-19` 和 `DEEPGEMM-2026-06-19` 是 DeepSeek MoE kernel infrastructure 的通信/计算两半。DeepEP 负责 token 在 expert-parallel domain 里的 dispatch/combine，DeepGEMM 负责 expert MLP、grouped GEMM、indexer 和 Mega MoE。Mega MoE 进一步把 EP dispatch、两个 FP8xFP4 GEMM、SwiGLU 和 EP combine 融合并 overlap。
- `DEEPEP-2026-06-19`、`DEEPGEMM-2026-06-19` 和 [2026-04-24](/papers/2026-04-24-deepseek-v4-million-token-context-intelligence/) 构成 DeepSeek-V4 kernel stack 的公开可读底层。V4 报告给出系统结果和模型架构，DeepEP/DeepGEMM 给出通信与 tensor-core kernel 的接口、benchmark 和版本面。
- `DEEPEP-2026-06-19` / `DEEPGEMM-2026-06-19` 和 [2205.14135](/papers/2205.14135-flashattention-io-aware-exact-attention/) / [2307.08691](/papers/2307.08691-flashattention-2-parallelism-work-partitioning/) 共同构成大模型 kernel 基础层。FlashAttention family 提供 attention IO-aware / parallelism 语言，DeepEP/DeepGEMM 提供 MoE communication、low-precision GEMM、scale-factor layout 和 fused expert path 语言。
...
- `2606.04101` 和 `DEEPEP-2026-06-19` 是 load balancing layer 与 communication primitive 的关系。DeepEP 优化既定 routing 下的 dispatch/combine 和通信资源占用；UltraEP 在 gating 后使用 exact load 做 expert replica、quota 和 reroute planning，再调用底层 communication path 执行。
- `2606.04101` 和 `DEEPGEMM-2026-06-19` 是 expert placement 与 local expert compute kernel 的关系。UltraEP 关注 token 应该落到哪个 physical expert instance，DeepGEMM 关注落到实例后的 grouped GEMM / fused Mega MoE 计算效率。
