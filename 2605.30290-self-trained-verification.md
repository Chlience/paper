...
- 初始轮：`y_0 ~ G(. | x)`。
- 第 `r` 轮 verifier：`(v_r, f_r) ~ V(. | x, y_{r-1})`，其中 `v_r` 是 accept/reject，`f_r` 是反馈。
- 若 reject，生成器根据 `x, y_{r-1}, f_r` 生成修正版 `y_r`。
- loop 在 accept 或最大轮数 `R` 后停止。

STV 训练 verifier：

- teacher 分布：`V*(. | x, y_{r-1}, y*(x))`。
- student 分布：`V_theta(. | x, y_{r-1})`。
...
- 设置：Qwen3-8B 作为 base generator 和 verifier；DAPO math problems 按 Qwen3-8B 32-rollout pass@1 分成 Hard (`0 < pass@1 < 0.2`) 和 Hardest (`pass@1 = 0`)；每个 test bin 约 150 题；每题 32 条独立 V-R loop，最多 20 轮。
- 指标：每轮 pass@1。
