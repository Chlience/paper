...
- [Wenhan Ma](/authors/wenhan-ma/): 发表时机构：Peking University / Xiaomi LLM Core；历史机构：Peking University。
- Jianyu Wei: 发表时机构：Xiaomi LLM Core。
- [Liang Zhao](/authors/liang-zhao-xiaomi/): 发表时机构：Xiaomi LLM Core；历史机构：Xiaomi MiMo / Peking University。
- [Hailin Zhang](/authors/hailin-zhang/): 发表时机构：Xiaomi MiMo / Xiaomi LLM Core；历史机构：Peking University。
- Bangjun Xiao: 发表时机构：Peking University / Xiaomi LLM Core。
- [Lei Li](/authors/lei-li-hku/): 发表时机构：Xiaomi LLM Core / The University of Hong Kong；历史机构：Peking University。
- Qibin Yang: 发表时机构：Peking University / Xiaomi LLM Core。
- Bofei Gao: 发表时机构：Peking University / Xiaomi LLM Core。
- Yudong Wang: 发表时机构：Peking University / Xiaomi LLM Core。
- Rang Li: 发表时机构：Peking University / Xiaomi LLM Core。
- Jinhao Dong: 发表时机构：Renmin University of China / Xiaomi LLM Core。
- [Zhifang Sui](/authors/zhifang-sui/): 发表时机构：Peking University；历史机构：Peking University。
- [Fuli Luo](/authors/fuli-luo/): 发表时机构：Xiaomi LLM Core；历史机构：Xiaomi MiMo / Peking University / DeepSeek AI。
...
- 解读：dense token-level teacher guidance 的数据效率更高。这里的优势来自 teacher 已经付过 domain RL 成本，因此应把 Stage 2 teacher training cost 计入整体研发成本，而非只看 MOPD Stage 3。
...
- Baseline fairness 依赖作者实现；开源代码或第三方复验前，Mix-RL / Cascade / Param-Merge 的弱点应作为方法动机，而非最终定论。
...
- 本文应归档为 Xiaomi MiMo / multi-domain post-training integration 节点，而非只归入普通知识蒸馏。
...
- 同源 teacher 条件是解释论文成败的主线，应在索引中明确连接到 OPD / parallel OPD / on-policy distillation 类方法。
...
- 与 [VERL](/papers/2026-06-16-verl-rl-optimization-algorithms/) / [slime](/papers/2026-06-17-slime-rl-scaling-framework/)：MOPD 的 PG 形式可接入 GRPO/PPO 类框架，teacher prefill service 类似 reward service；verl / slime 提供这类 post-training dataflow、rollout correction 和 async pipeline 的工程背景。
- 新增后应更新的索引 cluster：新增 Xiaomi MiMo / MOPD / multi-teacher on-policy distillation cluster，并在国产前沿模型技术路线中加入 MiMo-V2-Flash post-training integration 节点。
...
Why: MOPD 是 Xiaomi MiMo post-training 能力整合的核心方法节点，补齐当前档案中“多领域 RL teacher 如何整合为单 student”的缺口。它和 DeepSeek-V4 OPD、GLM-5.2 parallel OPD、Qwen3 strong-to-weak distillation、Math-Shepherd token/step-level signal 都有可追踪关系，适合纳入 papers-index 的国产模型 post-training 图谱。
