...
- [Zeyu Jia](/authors/zeyu-jia/): MIT EECS PhD candidate，主页显示中文名为贾泽宇，研究方向为 machine learning theory、reinforcement learning theory、optimization 和 information theory；OpenReview 确认其 MIT / PKU 背景，以及 [Alexander Rakhlin](/authors/alexander-rakhlin/) 和 Yury Polyanskiy advisor 关系。
- [Alexander Rakhlin](/authors/alexander-rakhlin/): MIT Professor，MIT Statistics and Data Science Center / IDSS、Department of Brain & Cognitive Sciences 和 LIDS；主页显示研究覆盖 machine learning、statistics、optimization、online prediction、decision-making 和 reinforcement learning。
- [Tengyang Xie](/authors/tengyang-xie/): University of Wisconsin-Madison Computer Science Assistant Professor；主页显示其此前为 Microsoft Research New England / NYC postdoc、UIUC PhD，研究方向覆盖 reinforcement learning、long-horizon planning、decision-making、world models 和 multimodality。

关系判断：

- 同机构作者群：Zeyu Jia 和 Alexander Rakhlin 同属 MIT，且存在 PhD advisor 关系；二者在 OpenReview 上还与 contextual bandit、offline RL 和 LLM reasoning 后续论文重复合作。
- 跨机构桥接：Tengyang Xie 连接 UW-Madison、Microsoft Research、UIUC / Nan Jiang offline RL 理论线，并与 Alexander Rakhlin 在 preference optimization、offline RL 和 reasoning RL 方向已有共同论文。
- 与已存档作者重叠：本次为三位作者新建作者页；当前已存档论文中未发现直接作者重叠。
- 与已存档论文的主题或方法关系：这篇直接回应 [2305.20050 Let's Verify Step by Step](/papers/2305.20050-lets-verify-step-by-step-process-supervision/) 所代表的人工 process supervision 优势叙事，也为 [2312.08935 Math-Shepherd](/papers/2312.08935-math-shepherd-automatic-process-supervision/) 一类 outcome-to-process automatic labeling 提供理论背景；与 [2605.25507 SRPO](/papers/2605.25507-credit-assignment-resets-language-model-reasoning/)、[2602.07078 OTB](/papers/2602.07078-optimal-token-baseline-long-horizon-llm-rl/) 和 [2606.20008 VIMPO](/papers/2606.20008-vimpo-value-implicit-policy-optimization-llms/) 共享 long-horizon credit assignment 主题。
- 作者 profile pass：按 SOP 尝试 Grok broad search，因 `SuperGrok Heavy subscription required` 403 失败，已回退到网页、OpenReview、DBLP、Scholar、主页和 X 搜索。Zeyu Jia 与 Tengyang Xie 找到高置信 X 候选；Alexander Rakhlin 未找到高置信个人 X，搜索结果中同名 Kaggle / GitHub 账号与 MIT 教授身份不匹配，已在 `authors.json` 标记 `not-found`。
...
- 与 `papers-index.md` 中已有论文的作者关系：当前没有直接作者重叠；本次新增 [Zeyu Jia](/authors/zeyu-jia/)、[Alexander Rakhlin](/authors/alexander-rakhlin/) 和 [Tengyang Xie](/authors/tengyang-xie/) 作者档案。
...
- 新增后应更新的索引 cluster：扩展 `Cluster AW: Process Supervision、PRM800K 与 Step-Level Verifier`，加入理论反思节点；在跨论文关系中把它接到 PRM800K、Math-Shepherd、SRPO、OTB、VIMPO 和 RLVR boundary 争论。
...
- Proposed form: 新建独立 Markdown 文档 `2502.10581-do-we-need-to-verify-step-by-step-process-supervision-theory.md`，同步索引表、Process Supervision cluster 和跨论文关系。
...
| Grok author search | 本地 Grok CLI 返回 403 `SuperGrok Heavy subscription required`，已回退到手动网页 / OpenReview / DBLP / Scholar / X 搜索。 |
| Alexander Rakhlin 个人 X | 未发现高置信 MIT professor Alexander Rakhlin 的个人 X；同名 Kaggle/GitHub/X 线索与其 MIT 教授身份不匹配。 |
