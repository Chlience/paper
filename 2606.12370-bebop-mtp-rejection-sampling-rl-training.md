...
其中 target distribution $p$ detached，梯度只流入 draft head $q$。对 draft logits $z_j$，梯度为：

$$
\frac{\partial \mathcal{L}_{\mathrm{TV}}}{\partial z_j}
=-q_j\left[\mathbbm{1}[q_j\le p_j]-S\right]
$$

$$
S=\sum_v \mathbbm{1}[q_v\le p_v]q_v
$$

它的关键性质是梯度与 $q_j$ 成比例。低概率 tail token 的梯度自然接近 0，优化压力集中在 draft 已经认为可能的 token 和 acceptance decision boundary 附近。
