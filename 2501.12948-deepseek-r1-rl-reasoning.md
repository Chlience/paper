...
- 与已存档作者重叠：已确认与 [Math-Shepherd](/papers/2312.08935-math-shepherd-automatic-process-supervision/) 共享 [Peiyi Wang](/authors/peiyi-wang/)、[Runxin Xu](/authors/runxin-xu/)、[Zhihong Shao](/authors/zhihong-shao/)、[Damai Dai](/authors/damai-dai/)、[Deli Chen](/authors/deli-chen/) 和 [Yu Wu](/authors/yu-wu-deepseek/)；与 [DeepSeek-V2](/papers/2405.04434-deepseek-v2-mla-moe-efficient-llm/)、[DeepSeek-V3](/papers/2412.19437-deepseek-v3-technical-report/) 和 [DeepSeek-V4](/papers/2026-04-24-deepseek-v4-million-token-context-intelligence/) 共享 [Wenfeng Liang](/authors/wenfeng-liang/)、[Huazuo Gao](/authors/huazuo-gao/)、[Wangding Zeng](/authors/wangding-zeng/)、[Xingkai Yu](/authors/xingkai-yu/) 和 [Zhenda Xie](/authors/zhenda-xie/) 等 DeepSeek 系列作者线索。
...
- 作者 profile pass：R1 是 200 人级别团队论文；本轮复用已建档核心作者与 DeepSeek 系列作者档案，新增链接覆盖当前档案中已确认身份的作者。未对完整作者表批量建档，原因是 core contribution / cross-paper relation 已能支撑本篇索引判断。
...
第一步是验证 pure outcome RL 的上限：从 DeepSeek-V3-Base 直接开始 RL，去掉 reasoning SFT，reward 只看答案正确性和基本格式。这样可以观察强 base model 在 verifier 驱动下是否会自发增加推理长度、反思、验证和策略切换。

第二步是把 raw reasoning capability 转成可用产品形态：R1-Zero 证明 pure RL 能诱导能力后，作者再加入 cold-start SFT、rejection sampling、general SFT 和 safety/helpfulness reward，把可读性、语言一致性、指令跟随和安全控制补回来。
...
5. 相关工作与表述：审稿人要求扩展 scaling laws、CoT、evaluation methodology、distillation scaling 等相关工作，并避免过强的智能化表述。这个意见提示读者把 R1 放在 scaling、verifier、distillation 和 RLHF/RLVR 的连续谱上看。
...
- 索引状态：`papers-index.md` 已包含 “Pure RL Reasoning Models 与 DeepSeek-R1 系谱” cluster，并把 R1 连接到 DAPO、TIM/VeXact、tool-calling RL、SocioHack、STV 和 inference determinism。
