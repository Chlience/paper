...
- 需要后续确认：若后续分析 FlashAttention-2/3/4，需要拆分为独立节点；本文记录的是 2022 年 FlashAttention v1 论文。
...
- 论文实验基于 2022 年 GPU、CUDA kernel、A100/T4/RTX 3090 等硬件；后续 FlashAttention-2/3/4 在 parallelism、Hopper/H100、FP8/低精度和 CuTeDSL 上继续演进。
...
- 与 approximate attention 的比较依赖当时实现质量；后续 Triton、xFormers、PyTorch SDPA、FlashAttention-2/3 等会改变 baseline。
...
- 与 PyTorch SDPA、FlashAttention-2/3/4、FlashInfer、FlexAttention 的当前 baseline 对比。
...
4. 实验硬件和软件栈是 2022 年环境，当前 baseline 已经包括 FlashAttention-2/3/4、PyTorch SDPA、Triton kernels、FlashInfer、PagedAttention 等。
...
| Material | Reason |
| --- | --- |
| FlashAttention-2/3/4 详细分析 | 这些是后续独立论文/技术报告，当前任务只分析 `2205.14135` |
