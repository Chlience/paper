...
- 方法详细度以读者能够复述完整执行链或论证链为验收标准，不设统一字数。方法包含多个关键阶段、角色或策略变体时，先给端到端概览并区分容易混淆的同名对象，再逐阶段说明输入或初始条件、核心操作、传递对象、输出或训练信号、设计理由、直接证据和失败边界。
- `论文脉络` 的详细方法从 `### 5.` 开始连续编号，省略只用于包裹子章节的“核心贡献与方法逻辑”标题。首节给出贡献全景与端到端执行链，后续 `### 6.`、`### 7.` 等按执行或论证依赖命名；结论链条等后续章节使用下一个未占用的整数。方法本身只有一个实质章节时可以保留单个 `### 5. 方法…`。
