...
- Chaofan Lin: Shanghai Jiao Tong University；脚注说明部分工作完成于 Microsoft Research internship。
- Zhenhua Han: Microsoft Research；论文 acknowledgments 后说明其为 corresponding author。
- Chengruidong Zhang: Microsoft Research。
- Yuqing Yang: Microsoft Research；论文 acknowledgments 后说明其为 corresponding author。
- Fan Yang: Microsoft Research。
- Chen Chen: Shanghai Jiao Tong University；脚注说明其部分工作完成于 Microsoft Research visiting scholar 期间；论文 acknowledgments 后说明其为 corresponding author。
- Lili Qiu: Microsoft Research。

关系判断：

- 同机构作者群：Zhenhua Han、Chengruidong Zhang、Yuqing Yang、Fan Yang、Lili Qiu 属 Microsoft Research；Chaofan Lin 与 Chen Chen 属 Shanghai Jiao Tong University。
- 跨机构桥接：Chaofan Lin 的 Microsoft Research internship 和 Chen Chen 的 Microsoft Research visiting scholar 经历连接 SJTU 与 MSR。
- 项目组织：ParrotServe 由 Microsoft 开源；GitHub README 将其标注为 OSDI 2024 research prototype，并说明当前不活跃维护。
- 与已存档作者重叠：未发现与当前存档论文直接作者重叠。机构层面与 [2504.13837](/papers/2504.13837-rlvr-reasoning-boundary-base-model/) 共享 Shanghai Jiao Tong University 网络；与 [2605.31514](/papers/2605.31514-age-of-empires-anthropomorphism/) 共享 Microsoft 机构网络，但研究方向不同。
- 与已存档论文的主题或方法关系：与 [2606.06453](/papers/2606.06453-vortex-sparse-attention-serving/) 同属 LLM serving systems；与 [2409.19256](/papers/2409.19256-hybridflow-rlhf-framework/) 都把上层 workflow / dataflow 显式交给系统调度器；与 [2606.00135](/papers/2606.00135-agentic-tool-calling-rl-training/) 共享 agentic workflow 与 harness structure 主题；与 [2025-09-10](/papers/2025-09-10-defeating-nondeterminism-llm-inference/) 和 [2605.14220](/papers/2605.14220-training-inference-mismatch-llm-rl/) 在 serving backend、batching 和 scheduler 会改变上层行为的意义上相关。
- 需要后续确认：ParrotServe 后续维护状态、是否有 production deployment 公开报告、是否已适配更现代的 vLLM/SGLang/prefix-cache 实现。
...
- Intended target system: 新增论文笔记；更新 `papers-index.md` 的 LLM application serving / systems cluster。
