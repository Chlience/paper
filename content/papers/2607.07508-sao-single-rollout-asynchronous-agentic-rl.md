...
Updated-At: 2026-07-21 16:47
Review-Status: needs-review
Reviewed-At: 2026-07-21 14:36
...
pilot observation 显示 critic 的 attention gradient norm 明显高于 MoE layer。作者把它视为优化不稳定性的定位信号，据此冻结 value model attention，只训练 MoE projections，并假设预训练 attention 已能从长上下文中选择 relevant state，critic 主要需要调整 state-to-value mapping。
...
Figure 4(b): full-parameter critic 的 gradient norm 从约 5.5 上升到 10 以上；frozen-attention critic 大致维持在 3 到 5。较低 gradient norm 与更平滑曲线支持优化稳定性，单凭 norm 高低还不能证明 value prediction 更准确。Image Source: [arXiv HTML image / Figure 4(b)](https://arxiv.org/html/2607.07508v1/x7.png).
...
高 attention gradient norm 表示 value loss 对这组参数给出了较强的聚合更新信号。原始数值还会受到参数量、参数尺度、gradient clipping 和 optimizer preconditioning 影响，因此跨模块比较时最好同时报告 per-parameter gradient RMS 与 relative update norm。作者将高 norm、full-parameter critic 的性能下降和 frozen-attention 曲线更平滑放在一起，形成冻结 attention 的经验依据；这些结果仍不足以单独验证“预训练 attention 已经完成 relevant state selection、MoE 只需学习 state-to-value mapping”这一机制假设。
