...
- Version / revision read: arXiv v3 PDF、TeX source；OpenReview submission forum、4 份 official reviews、4 份 rebuttals 与 decision；GitHub commit `7448751f307a9cdbcc1246dd1565a1a605b443df`
- Accessed: 2026-07-28
...
3. 教师上下文引起的概率变化主要包含解题信息；思考模式和提示风格引起的变化可以由提示配置与裁剪控制。
...
论文直接检验了不同散度、思考模式、裁剪、生成长度和完整词表/采样 token 两种目标。教师对错误前缀的纠错质量、提示变化的鲁棒性、参考解答质量和总 FLOPs 没有得到直接评测。
...
教师提示要求模型理解参考解答后用自己的方式重新求解。教师实际不生成一条新解答；程序把学生轨迹接到教师提示后，通过一次预填充和前向计算读取各位置 logits。因此，论文中的“rationalization”表示特权上下文对隐藏状态和下一 token 分布的条件化效果，当前流程没有显式产出或验证一条教师推理。

主实验将学生设为 thinking-mode off，将教师设为 thinking-mode on。学生 rollout 因而直接进入回答模式，教师评分上下文保留思考模板。该配置在三种 Qwen3 规模上产生最大的数学 token KL，同时也产生更大的风格 token KL。
...
3. **直接证据**：三个 Qwen3 规模的三项数学 Avg@12 均高于基础模型、SFT 与 GRPO 的模型级平均；散度、思考模式、裁剪、长度和目标形式消融支持主要配方。
...
- Baseline：相同训练设置下的三种散度、四种思考模式组合和无裁剪变体。
- 对照是否可比：散度比较共享裁剪方案；论文没有报告 $\tau$ 搜索。思考模式分析只使用 10 道题，style/math 类别来自人工关键词表，类别 KL 与最终正确率之间没有因果分解。裁剪曲线只展示 1.7B/AIME24 单一设置。
- 证据定位：Section 4.3.1–4.3.3；Tables 3、5；Figure 4；Appendix B–C；PDF pp. 7、13–14；repository `opsd_trainer.py:382-479` 与 `scripts/run_opsd_*.sh`。
- 支持的最窄结论：在本文小规模消融中，前向 KL 最有效，逐词表项裁剪能稳定一个 1.7B 训练设置。
- 解读：风格 KL 显著高于数学 KL，直接说明上下文差异同时传递风格信号。裁剪提供可用的工程控制，当前证据还不足以确认其阈值可跨模型、数据和提示迁移。
...
8. **提示与思考模式是强混杂因素。** TM-off 学生/TM-on 教师同时改变模板、风格与可见推理结构；10 道题的关键词 KL 分类无法分离数学知识和提示风格。
...
4. 提示上下文差异会同时传递任务知识和风格信号。按 token 类别监测散度、控制极端分量和验证提示鲁棒性，应成为特权上下文蒸馏的标准诊断。
