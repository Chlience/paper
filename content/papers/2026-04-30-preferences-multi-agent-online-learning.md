...
同步更新更适合寻找共同部署时互相适配的策略，因为一次成功的联合轨迹可以同时更新所有参与角色。它能否从次优稳定点转移到更高奖励区域，还取决于更新方向、探索相关性和步长。继续使用两个角色的共同奖励例子：双方都选 A 时奖励为 6，都选 B 时奖励为 10，选择不一致时奖励为 0。令角色 $i$ 选择 B 的概率为 $p_i$，独立随机化下的期望奖励为

$$
J(p_1,p_2)
=6(1-p_1)(1-p_2)+10p_1p_2
=6-6p_1-6p_2+16p_1p_2.
$$

两个策略各自看到的局部方向为

$$
\frac{\partial J}{\partial p_1}=-6+16p_2,
\qquad
\frac{\partial J}{\partial p_2}=-6+16p_1.
$$

同步更新表示在同一个联合策略 $(p_1,p_2)$ 上计算这两个偏导，并在同一步中同时应用。当 $p_1,p_2<3/8$ 时，两个偏导都推动 B 的概率下降，轨迹会回到 $(A,A)$ 附近。沿对称路径 $p_1=p_2=p$ 移动时，

$$
J(p,p)=6-12p+16p^2.
$$

从 $p=0$ 开始小幅提高双方的 B 概率会先降低期望奖励，直到跨过 $p=3/8$ 附近的分界。这里形成协调低谷：$(B,B)$ 的奖励更高，确定性小步梯度仍会把低 B 概率的策略带回 $(A,A)$。每个偏导在数学上都把另一策略暂时固定在其当前混合分布；同时应用多个偏导不会自动生成“双方直接改用 B”的联合动作提议。

有限样本探索增加了一条逃离路径。若两个角色各自以概率 $p$ 独立探索 B，成功采到 $(B,B)$ 的概率为 $p^2$；这次高奖励轨迹可以同时强化两个策略。选择不一致的概率为 $2p(1-p)$，在 $p$ 很小时，协调成功事件按二阶概率出现，选择不一致事件按一阶概率出现。扩展到 $n$ 个角色时，所有角色同时选中目标方案的概率可以缩小到各自探索概率的乘积。有限批次噪声造成的方向波动、保持探索概率的熵奖励、显式联合扰动或多次重启有机会让轨迹越过分界，期望动力学并不保证这种转移发生。

相关探索（correlated exploration）会直接改变这个概率结构。假设所有角色共享一个随机变量：以概率 $1-q$ 一起选 A，以概率 $q$ 一起选 B。此时只访问两个协调组合，期望奖励变成

$$
J_{\mathrm{corr}}(q)=6(1-q)+10q=6+4q.
$$

提高 $q$ 会立即提高期望奖励，因为探索过程跳过了选择不一致的组合。共享随机种子、可通信的协调变量、中心化联合策略或显式联合动作选项都可以实现近似机制。论文采用 $x_\alpha=\prod_i x_{i,\alpha_i}$ 的独立混合策略模型，相关联合分布超出了它的直接定理范围。

从论文的稳定性结果看，$(A,A)$ 与 $(B,B)$ 都是严格纳什均衡，也是单点 club 集合；Theorem 3 允许两者都成为局部渐近稳定点。定理可以解释训练为何停在奖励为 6 的点，也可以解释奖励为 10 的点为何能保持稳定；团队奖励排序需要另行比较。Proposition 2 进一步说明，一般 club 集合的混合内部可能在耦合动力学下失稳并离开其张成区域。这项反例确立了同步耦合运动能够改变长期集合行为。应用到共同奖励任务时，离开方向的团队收益仍需单独评估，结果可能更高，也可能更低。复制子动力学中的 $\ell_\alpha(x)>0$ 表示外部联合组合 $\alpha$ 的概率在当前位置具有正的对数增长率，它属于局部聚合漂移指标；全局奖励比较需要另行完成。

训练方案的选择应随目标变化：

- 最终目标是让固定的一组角色共同部署时，联合训练通常更合适，因为它允许策略共同适应，并能让成功联合轨迹同时更新多个角色。
- 目标是识别单个角色的因果贡献、保持搭档环境稳定或评估跨搭档稳健性时，冻结其他策略更合适。
- 目标是越过协调低谷时，关键设计变量是相关探索、联合动作提议、初始化覆盖和跨分界步长；同步应用若干独立策略梯度只提供其中一部分条件。

因此，一个更完整的实验方案是以联合训练作为共同部署性能的主设置，同时保留冻结搭档的归因评估，并单独比较独立探索与相关探索。这样可以区分“多策略共同适应带来的收益”和“探索机制成功跨过协调低谷带来的收益”。

### 7. 后续复验指标

- 对多角色系统同时运行四种设置：单角色更新、其他角色更新、交替更新、全部角色同步更新；使用同一固定搭档池和共同任务集评估，分别报告条件贡献与联合系统收益。
- 从同一个次优严格纳什均衡附近初始化，比较冻结式坐标更新、交替更新、独立探索下的同步更新、相关探索下的同步更新和中心化联合策略；报告跨越分界的种子比例、联合动作覆盖率、选择不一致率、样本量与最终团队奖励。
...
- 同步更新扩大了共同适应和联合轨迹强化的机会；独立策略的确定性小步方向仍由单边偏导组成，跨越协调低谷通常还需要随机联合探索、相关探索或显式联合动作更新。
...
10. 论文用各玩家独立随机化的乘积分布描述联合策略；共享随机变量、相关探索、中心化联合策略和有限批次偶然越过吸引域边界的概率不在定理覆盖范围内。
