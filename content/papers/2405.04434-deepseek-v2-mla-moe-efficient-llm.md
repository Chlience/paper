...
V2 的实际维度是 $d_h=128$、$d_h^R=64$、$d_c=512$。标准 MHA 每层、每 token 缓存 $2n_hd_h=32{,}768$ 个元素，MLA 缓存 $d_c+d_h^R=576$ 个元素，persistent state 约缩小 56.9 倍。absorbed content score 和 latent value aggregation 对每个 head 使用 512 维 latent，标准 MHA 的对应 head dimension 是 128；MLA 的 attention arithmetic 因此未必更少。它的系统收益主要来自历史状态读取量、可容纳 batch/context 和 memory-bound decode，仍保留 $O(T)$ decode 历史扫描与 $O(T^2)$ prefill attention。
