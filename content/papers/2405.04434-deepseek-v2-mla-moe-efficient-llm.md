...
#### MLA 的完整前向：Q/K/V 如何进入 Attention

论文正文先给出 MLA 的架构定义，Appendix C 再给出完整公式。以下统一采用列向量；$W_i^{UQ}$、$W_i^{UK}$、$W_i^{UV}$ 和 $W_i^{QR}$ 表示相应矩阵切给第 $i$ 个 head 的 block。对第 $t$ 个 token 的 attention 输入 $h_t\in\mathbb{R}^{d}$，query 路径是：

$$
\begin{aligned}
c_t^Q &= W^{DQ}h_t &&\in\mathbb{R}^{d_c'},\\
q_{t,i}^C &= W_i^{UQ}c_t^Q &&\in\mathbb{R}^{d_h},\\
q_{t,i}^R &= \operatorname{RoPE}_t\!\left(W_i^{QR}c_t^Q\right)
&&\in\mathbb{R}^{d_h^R}.
\end{aligned}
$$
...
把 V2 的实际超参数代入后，单 token、单层的维度流如下；表中省略 batch 和 sequence 维：

| 路径 | 维度变化 | 产生的张量 | Generation cache |
| --- | --- | --- | --- |
| Attention 输入 | - | $h_t:[5120]$ | 否 |
| Query bottleneck | $5120\rightarrow1536$ | $c_t^Q:[1536]$ | 否 |
| Content queries | $1536\rightarrow128\times128$ | $q_t^C:[128,128]$，共 16,384 elements | 否 |
| RoPE queries | $1536\rightarrow128\times64$ | $q_t^R:[128,64]$，共 8,192 elements | 否 |
| Joint KV bottleneck | $5120\rightarrow512$ | $c_t^{KV}:[512]$ | **是，512 elements** |
| Content keys | $512\rightarrow128\times128$ | $k_t^C:[128,128]$，共 16,384 elements | optimized path 不物化、不缓存 |
| Content values | $512\rightarrow128\times128$ | $v_t^C:[128,128]$，共 16,384 elements | optimized path 不物化、不缓存 |
| Shared RoPE key | $5120\rightarrow64$ | $k_t^R:[64]$，所有 heads 共享 | **是，64 elements** |

因此 $c_t^{KV}$ 是所有 128 个 heads 共享的一份 512 维表示，它共同参数化逻辑上的 16,384 维 content keys 和 16,384 维 content values。query 会在当前 step 使用后释放，不形成历史 cache；历史 token 只留下 $c_t^{KV}$ 和 $k_t^R$：
...
其中 $C$ 表示 content 子空间，$R$ 表示承载 RoPE 的 position 子空间。每个 head 的完整 query/key 是拼接向量：
...
MLA 随后的 Attention 仍采用 causal scaled dot-product Softmax。对历史位置 $j\le t$：

$$
\ell_{t,j,i}
=\frac{q_{t,i}^{\top}k_{j,i}}{\sqrt{d_h+d_h^R}}
=\frac{
\underbrace{(q_{t,i}^C)^{\top}k_{j,i}^C}_{\text{content logit}}
+\underbrace{(q_{t,i}^R)^{\top}k_j^R}_{\text{position logit}}
}{\sqrt{d_h+d_h^R}},
$$

$$
a_{t,j,i}=\operatorname{Softmax}_{j\le t}(\ell_{t,j,i}),\qquad
o_{t,i}=\sum_{j=1}^{t}a_{t,j,i}v_{j,i}^C,
$$
...
这组公式给出 MLA 对标准 MHA 的直接改动：Q 的 content 路径经过 $d\rightarrow d_c'\rightarrow n_hd_h$ 低秩投影；K/V 先共享 $d_c$ 维 latent，再通过不同 head block 形成 content K/V；position key 在所有 heads 之间共享，position query 保持 head-specific；content logit 与 position logit 相加后进入同一次 Softmax，所得权重再作用于 content value。MLA 因而保留 $n_h$ 组 attention weights、causal mask 和对全部历史位置的访问，它改变的是 Q/K/V 参数化与缓存表示。

#### Decoupled RoPE：把位置能力放进小分支

MLA 的关键难点在 RoPE。若对 content key 直接施加 `RoPE (Rotary Position Embedding，旋转位置编码)`，上投影矩阵和当前位置相关的旋转矩阵会夹在 query 侧矩阵之间，推理时无法把 key 的上投影吸收到 query projection 里，低秩 KV 压缩会在 decode 阶段重新引入展开成本。V2 用 decoupled RoPE 把 key/query 拆成 content 分支和 position 分支：content 分支从 latent KV 生成，不直接承载 RoPE；position 分支单独生成 64 维 RoPE key/query。cache 中只保存 compressed latent KV 和 decoupled RoPE key，位置关系通过上式的 position logit 进入同一组 attention weights。这是 V2 相对传统 MHA/GQA 的核心新机制。

#### Projection absorption：把历史 key 展开移到当前 query

上面的完整前向是论文的架构定义；若按它逐项执行，仍需把每个历史 $c_j^{KV}$ 展开成 per-head $k_{j,i}^C$ 和 $v_{j,i}^C$。deployment path 利用矩阵乘法结合律重排计算。content score 可以写成：
...
则部署时的完整 logit 直接作用于缓存中的 latent 和 RoPE key：

$$
\ell_{t,j,i}
=\frac{
(\widetilde q_{t,i}^C)^\top c_j^{KV}
+(q_{t,i}^R)^\top k_j^R
}{\sqrt{d_h+d_h^R}}.
$$

value/output 侧也可以在 latent 空间完成。把 $W^O$ 按 heads 分成 $W_1^O,\ldots,W_{n_h}^O$，并定义：
...
因此 optimized decode 的执行顺序是：计算当前 token 的 $c_t^{KV}$ 与 $k_t^R$ 并写入 cache；计算各 head 的 $\widetilde q_{t,i}^C$ 与 $q_{t,i}^R$；分别与历史 latent、RoPE key 做矩阵乘并相加；执行一次 causal Softmax；用每个 head 的权重对历史 latent 求和；通过 absorbed output projection 生成 $u_t$。整个过程无需物化历史 per-head content K/V。

设当前 prefix 长度为 $T$，absorbed decode 的主要矩阵 shape 可以进一步写成：
...
对照标准 MHA，历史 K/V cache 分别是 $[T,128,128]$；MLA 的两份历史状态是 $C_{1:T}^{KV}:[T,512]$ 与 $K_{1:T}^R:[T,64]$。attention logits 和 weights 仍为 $[128,T]$，所以 head 数和历史位置数没有压缩；发生压缩的是被所有 heads 共享的持久 K/V 表示。

维度压缩集中在 persistent state。absorbed content score 和 latent value aggregation 对每个 head 使用 512 维 latent，标准 MHA 的对应 head dimension 是 128；MLA 的 attention arithmetic 因此未必更少。它的系统收益主要来自历史状态读取量、可容纳 batch/context 和 memory-bound decode，仍保留 $O(T)$ decode 历史扫描与 $O(T^2)$ prefill attention。

RoPE 分支必须独立保留。若把 RoPE 直接施加到 content key，位置相关矩阵 $R_j$ 会进入 $(q_{t,i}^C)^\top R_t^\top R_jW_i^{UK}c_j^{KV}$，其中 $R_j$ 随历史位置变化，$W_i^{UK}$ 无法预先吸收到当前 query 的固定投影中。decoupled RoPE 让 content 分支保持上述可结合结构，并通过单独的 position logit 保留相对位置信号。
