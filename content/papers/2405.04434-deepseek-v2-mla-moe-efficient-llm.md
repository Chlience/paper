...
MLA 的 93.3% 是模型级单份 cache 压缩口径。常见 absorbed tensor-parallel runtime 会在各 attention rank 复制不含 head 轴的 latent KV；展开 K/V 则可以沿 local heads 分片。由此产生的每 rank 收益、盈亏点和 DP Attention / DCP 等 ownership 方案集中记录在 [MLA TP cache sharding](/papers/2026-07-16-mla-tensor-parallel-cache-sharding/)。这项边界补充了模型报告到多卡 runtime 之间的第二个复现层级。
...
- 与 [MLA TP cache sharding](/papers/2026-07-16-mla-tensor-parallel-cache-sharding/)：该综合从 DeepSeek-V3 官方 naive / absorb cache shape 出发，说明 V2 的共享 latent 在纯 head-TP 下经常按 rank 复制，并把单份表示压缩扩展为包含副本数、序列 ownership、dtype 与 HBM residency 的分布式账本。
