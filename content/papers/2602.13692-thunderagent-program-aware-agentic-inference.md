...
ThunderAgent 把跨多轮 LLM 调用与工具等待建模为带 phase、status、KV footprint 和 tool dependency 的 LLM Program，使 scheduler 能按 Reasoning / Acting 状态执行 KV pause / restore，并在 KV 释放后通过全局队列选择恢复节点；论文在所测高并发 tool-use serving 与 RL rollout 中报告了更高的 steps/min，tool resource manager、time decay 与 STP 分解分别属于配套资源机制、可选调度启发和 accounting 框架。
...
3. paused program 的 KV 已经释放后，可以放入全局队列并在有容量的节点重新 prefill；这是释放 KV 后的重新 placement，并非 live KV 跨节点迁移。
...
5. 论文将该抽象实现为 vLLM / SGLang 上层 middleware；agent 侧需要 `program_id` 与 lifecycle signal，backend capacity 查询和 tool teardown / preparation 仍需相应集成点。
...
- 开源仓库、SkyRL 与 NVIDIA Dynamo 集成提供了实现和移植参考；完整复现仍需要固定 backend、orchestrator 与 workload 配置。
...
- tool resource manager 整体带来约 10% latency 改善和 $4.2\times$ disk memory savings；GC 与异步 preparation 未分别消融，结果依赖工具环境的初始化和清理策略。
