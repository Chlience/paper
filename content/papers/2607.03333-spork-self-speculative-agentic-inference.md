...
其中 $\ell_i$ 是第 $i$ 个 name token 的 top-1 logprob；首 token 的 opening quote 属于高概率模板字符，因此从 $i=2$ 开始。只要一个 name token 置信度低，整个 probe 就延后提交。

当 $c\geq\theta$ 时，controller 提前执行工具；若当前 probe 未过阈值，D2 会在 main 累积更多 CoT 后按 token cadence 重试。GAIA 的 997 个 probes 上，$\theta=0.90$ 达到 88% precision、100% recall、F1 0.937，并过滤 77% probes。D1 的 turn-level strict acceptance 约为 0.22，D1+D2 提升至约 0.37。
