...
D3 的 token-level 保真来自 target verification。论文实验中的 D3 使用 engine integration，把 probe draft 绑定到对应的 main request；公开仓库 commit `31d5ab6f...` 另外提供 `/spork/*` HTTP sidecar 作为机制示例，README 明确说明 paper D3 numbers 来自 engine mode。

HTTP 示例目前缺少 request-level 隔离。Controller 虽然向 `/spork/set_tokens` 发送 `request_id`，worker 端仍会先执行 `clear_all()`，再固定调用 `set_probe_tokens(0, ...)`。两个 agent request 并发时，后写入的 draft 会覆盖前一个，slot `0` 也可能对应当前 decode batch 中不同的请求。因此仓库要求 evaluation 使用 `workers=1`，含义是评测端一次只运行一个 agent request；tensor parallel 仍可配置为 TP>1。
...
3. D3 的论文结果来自 engine integration。公开 HTTP sidecar 固定使用 slot `0` 并在每次设置 draft 前清空全局 state，因此 `workers=1` 表示评测端只运行一个并发 agent request；它仍可使用 TP>1。默认 `NSPEC=20` 还会把更长 probe 截为前 20 tokens，而论文的 BrowseComp Phase 0 平均可复用前缀为 29.5 tokens。这条 reference path 需要补齐 request-level 隔离和版本验证后，才能复验 paper engine path 的并发行为与完整 D3 收益。
