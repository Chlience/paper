...
- $T_{\mathrm{oh}}$：rejected turn 的额外 probe compute 与错误 speculative execution 映射到 wall time 后的开销。
...
主文进一步近似为 $\alpha t_{\mathrm{overlap}} \geq T_{\mathrm{oh}}$。这个式子强调两个工程方向：提高 strict accepted overlap，或降低 rejected-turn overhead。
...
其中 $\ell_i$ 是论文定义的第 $i$ 个 name token 的 top-1 logprob。论文解释从 $i=2$ 开始是为了跳过 opening quote；这个解释与公开 prompt / code 不一致：Qwen 强制前缀已经以 `{"name": "` 结束，completion 的首 token 应进入工具名。对单 token 工具名，论文公式还会产生空集合。
...
当 $c\geq\theta$ 时，controller 提前执行工具；若当前 probe 未过阈值，main 继续 decode，D2 在 main 累积更多 CoT 后按 token cadence 发起一个使用更长 observed prefix 的新 probe。论文在 GAIA 的 997 个 probes 上用“tool name 是否匹配 main”作为标签，$\theta=0.90$ 达到 88% precision、100% recall、F1 0.937，并过滤 77% probes；阈值在同一批 probes 上按 F1 选择，没有 held-out calibration。D1 的 turn-level full-call strict acceptance 约为 0.22，D1+D2 提升至约 0.37，这个指标与 name-match classification 分属不同层级。
...
严格匹配失败经常发生在 argument 后缀。论文给出两组不同数据：Section 3.3 的 GAIA 两个 seeds 共 657 个 rejected events，中位匹配前缀为 18 tokens、均值 21.1、$P_{25}/P_{75}=14/25$；BrowseComp Phase 0 的 1268 个 probe-baseline pairs 中，首次 mismatch 位置均值为 29.5 tokens、中位数 27、$P_{25}/P_{75}=17/35$，82% 的 pairs 至少匹配 10 tokens，61% 至少匹配 20 tokens。这两组数字分别说明不同 workload 的 miss prefix，不能把 27 tokens 作为通用值。
...
D3 的 token-level 保真来自 target verification。论文实验中的 D3 使用 engine integration；公开仓库 commit `31d5ab6f...` 还提供 `/spork/*` sidecar 参考实现，README 明确说明 paper D3 numbers 来自 engine mode。该 sidecar 把状态放在 server-level `request_id=0`，要求 evaluation 使用 `workers=1`；设置新 draft 前会清空 proposer state，默认最多注入 20 个 draft tokens，并通过 monkey patch 接入 vLLM 的 `GPUWorker` 与 `build_app`。默认 20-token 配置无法覆盖论文按 29.5-token mean 估算的全部平均前缀；这个上限可以调整，版本兼容性与并发隔离仍需重新实现。
...
SPORK 的三项设计适合沿同一成本模型理解。D1 让 probe 尽量在关键路径之外完成，主要降低 rejected-turn overhead；D2 用更多 reasoning 信息换取更高 strict acceptance，同时缩短剩余 overlap window；D3 降低 miss 分支的 token decode。端到端收益最终取决于 $\alpha t_{\mathrm{overlap}}$ 能否覆盖 probe、错误工具执行和 serving contention。

这条关系也给出部署前的最小验证顺序：先测 early tool identity 与完整调用接受率，再测 probe wall time 和 remaining decode，最后在目标并发度下测 queueing、KV footprint 与 wasted tools。Qwen3-32B / GAIA 的 $P_{95}$ 下降 18% 是当前最清晰的整体证据；其它 16%–20% 结果来自不同模型、serving mode 与组件组合，适合作为迁移线索。

### 8. Figure 6 的时序边界

Figure 6 采用“main 上路 + speculative 下分支”的布局，四条泳道只能作为讲解时的时间对齐辅助。更完整的事件顺序如下：
...
- D1 在 main 首个 streaming token `t1` 后发起 forced-prefix probe。D2 实际可以从逐渐增长的 CoT prefix 发起多次短 probe；Figure 6 抽象为一次。
- 工具在候选 `Â` 可解析且 confidence gate 通过后启动，与剩余 CoT 和 main 的 tool-call decode 重叠。Figure 6 画出工具在 `main stop` 前返回的 accepted case；一般情况下它可以更早完成，也可以越过 `main stop`，后者仍需等待 residual tool time。
- D3 token verification 发生在 main 到达 `<tool_call>` boundary 时，probe body 作为 draft 在 main 的 tool-call decode 内接受 target-model verification。它只属于 engine D1+D2+D3 path。
- action-level strict verification 发生在 main 完整生成 `A*` 后。完整 tool name 与 serialized arguments 匹配时复用或等待预执行结果；发生 mismatch 时丢弃该结果并串行执行 `tool(A*)`。

因此，Figure 6 紫色的 `verify + accept` 对应 main-stop 处的 action-level strict compare；Figure 9 展示 D3 在 tool-call decode 内的 token-level verification。两个验证事件位于不同层级和时点。论文中的 `commit` 指 D2 gate 通过后 dispatch 推测工具，此时结果仍未进入 canonical trajectory。证据定位：Figure 6（PDF p.6）；Figure 9 与 Algorithm 1（PDF p.8）；Section 4–5。
...
3. D3 的论文性能来自 engine integration；公开仓库的 HTTP sidecar 使用 server-level `request_id=0`、要求 `workers=1`、清空全局 proposer state、默认最多注入 20 tokens，并 monkey-patch vLLM 内部 API。`requirements.txt` 也没有 pin vLLM，这条 reference path 不能替代 paper engine path 的并发或同版本复现。
