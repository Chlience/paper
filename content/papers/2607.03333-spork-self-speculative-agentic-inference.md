...
- 本文分析的是 inference-time controller 与 serving integration；SPORK 不训练模型，也没有 RL、SFT 或额外 predictor 参数。
...
- 解读：self-speculation 省去第二个 served model；同模型额外 request 的 capacity cost仍需在高并发下计入。
...
- Overlap with existing assets: token-level correctness 关系链接到 Speculative Decoding；agent-serving 调度关系链接到 ThunderAgent，本文保留 action-level overlap 的独立贡献。
