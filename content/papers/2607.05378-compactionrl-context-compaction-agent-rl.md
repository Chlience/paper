...
CompactionRL 让同一个 trainable actor 在上下文预算将满时生成摘要，再从“摘要 + 最近两轮”重建上下文，并把执行 token 与摘要 token 一起放入共享最终任务 reward 的 PPO 目标；独立 critic、全 batch token-level loss normalization 和跨 segment GAE 修正使 variable-length compacted rollouts 可训练。最接近的 SUPO 已经联合优化摘要与执行并使用 rollout-level group advantage，本文更明确的增量集中在 group size 1 的 critic 路线、token 级时序 credit 修正和 coding-agent 规模实验，尚缺与 SUPO、ReSum-GRPO、Context-Folding 或 ECHO 的直接对照。
...
训练使用 SWE-Dev 数据与 [slime](/papers/2026-06-17-slime-rl-scaling-framework/) asynchronous RL framework。论文没有公开 queue、partial rollout、weight sync、policy lag 或 staleness correction 配置，因此目前只能确认框架选择，不能重建完整异步执行语义。
...
- 与 [slime](/papers/2026-06-17-slime-rl-scaling-framework/)：论文明确使用 slime 完成 asynchronous RL training，是该框架支持 custom agent rollout、critic 和 token loss 的直接下游案例；具体 queue 与 weight-sync 配置仍未公开。
...
- 与 [RL Credit Assignment Watch](/papers/2026-07-13-rl-credit-assignment-may-july-landscape/)：该综合将 CompactionRL 归入 compressed-state critic 路线，并与 ECHO 的 provenance mask、Memory-R2 / HiMPO 的 same-state local comparison 对照；其优势是 signed temporal credit，主要估计风险来自 critic calibration 与 boundary bootstrap。
...
- Overlap with existing assets: 与 ECHO、SAO、GLM-5.2 和 slime 关系紧密；本节点聚焦 shared-summary PPO 和 cross-segment advantage。
