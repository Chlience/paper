...
- 与 [VERL](/papers/2026-06-16-verl-rl-optimization-algorithms/) 的方法关系：Laminar 内部 trainer / rollout 建立在 verl 上。VERL V1 公开 TransferQueue、ReplayBuffer、版本指标和多类异步模式；Laminar 给出 CPU/RDMA relay、same-version repack 与 1024-GPU 系统实验，但公开数据契约和 staleness policy 更少。
...
- 与 [slime](/papers/2026-06-17-slime-rl-scaling-framework/) 的数据契约关系：slime 公开 token-aligned Sample、shared rollout identity、behavior logprob 和 fully async queue；Laminar 更强调 relay、repack 和 fault isolation。两者结合显示 async RL 同时需要 trajectory provenance 与大规模参数 / 尾部调度。
