...
### 8. 双支柱：Externalized Harness 与 Internalized Optimization
...
- **与 context/memory 系统的关系**：[SelfCompact](/papers/2606.23525-self-compacting-language-model-agents/) 与 [SmoothAgent](/papers/2607.00151-smoothagent-lookahead-context-engineering/) 分别从 policy 侧的 trajectory summary 和 serving runtime 侧的异步 transform/KV cache 处理 C2 问题。它们把本篇 H2 的压缩与恢复要求落实到两个系统位置。
- **与 agentic environment 的关系**：[LLM-in-Sandbox](/papers/2601.16206-computer-environments-agentic-intelligence/) 提供可执行计算机接口；[Agentic RL 的可学习环境演进](/mainlines/agentic-rl-learned-environment-evolution/) 进一步区分真实沙箱、可执行环境生成与学习型转移。它们为本文 Optimization §5.2 和真实交互前沿提供更细的环境真实性与 sim-to-real 判断轴。
