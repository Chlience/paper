...
Figure 3 使用一组简化记号比较三种训练方式：$t_i$ 表示位置 $i$ 的 token，$f_i$ 表示目标模型在 LM head 之前的顶层隐藏状态，$a_i$ 表示解除特征回归约束后由草稿模型产生的输出向量。帽号表示模型预测，例如 $\hat{f}_{i+1}$ 是原始 EAGLE 预测的下一位置特征，$\hat{t}_{i+1}$ 是预测的草稿 token，$\hat{a}_{i+1}$ 是解除约束后的草稿输出。论文在后续流水线说明中省略了 $a$ 的帽号，本文也沿用 $a_i$ 表示已经由草稿模型算出的输出。图中还省略了每一步实际都会使用的已采样 token embedding。

EAGLE-3 将实际输入改为多层融合特征：对于已经由目标模型处理的位置，低、中、高层隐藏状态先融合为 $g_i$；$e_i$ 表示已采样 token $t_i$ 的 embedding，$D$ 表示单层草稿 decoder。第一轮的数据流可以写为：

$$
a_i = D(g_1,\ldots,g_{i-1};e_i),
\qquad
\hat{t}_{i+1} \sim \operatorname{softmax}(\operatorname{LMHead}(a_i)).
$$

第一轮输入中的 $t_i$ 只是目标 LM head 的采样结果，尚未作为输入经过目标模型主体，因此没有对应的 $g_i$；草稿输出 $a_i$ 占据这个位置。得到草稿 token $\hat{t}_{i+1}$ 后，下一轮取得它的 embedding $e_{i+1}$，数据流变为：

$$
a_{i+1} = D(g_1,\ldots,g_{i-1},a_i;e_{i+1}).
$$

因此，$f$ 是原始 EAGLE 与 Figure 3 比较示意中的顶层特征，$g$ 是 EAGLE-3 从目标模型多层信息得到的融合特征，$a$ 是草稿模型输出并在后续轮次回送的隐藏向量。训练使用专门的注意力 mask：原始训练序列保持因果依赖，同一轮并行产生的位置在后续轮次只读取对应路径，避免把互不构成同一前缀的候选混合起来。
...
Figure 3: 上部展示 EAGLE 的特征回归如何支持第二步递归；中部展示直接移除特征回归造成的后续步骤偏移；下部展示 EAGLE-3 把自生成隐藏状态纳入训练。图中的 $f$、$t$、$a$ 依次表示目标模型顶层特征、token 和不受特征回归约束的草稿向量，帽号表示模型预测。实际 EAGLE-3 使用融合特征 $g$ 取代 $f$，并同时输入已采样 token 的 embedding $e$。Image Source: [arXiv v3 Figure 3](https://arxiv.org/html/2503.01840v3/x4.png).

### 6. 多层目标特征融合

目标模型最后层隐藏状态紧邻 LM head，主要服务下一个 token 的 logits。更远 token 还可能需要中间层保留的词法、句法和语义信息。对于位置 $i$，EAGLE-3 把目标模型的低层、中层和高层隐藏状态分别记为 $l_i$、$m_i$ 和 $h_i$。若目标模型的隐藏维度为 $k$，三者都属于 $\mathbb{R}^k$；沿隐藏维拼接后得到 $3k$ 维向量，再经全连接层投影为融合特征：

$$
g_i = \operatorname{FC}([l_i;m_i;h_i]),
\qquad
g_i \in \mathbb{R}^k.
$$

这里的方括号表示向量拼接。融合特征 $g_i$ 与已采样 token 的 embedding $e$ 再次拼接并降回 $k$ 维，然后送入单层草稿 decoder；decoder 输出的 $a_i$ 同样是 $k$ 维向量，可以在下一轮替代尚未取得的 $g_i$。
