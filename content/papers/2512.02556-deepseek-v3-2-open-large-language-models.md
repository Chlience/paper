...
- 与 [Prefill CP](/papers/2026-07-14-prefill-context-parallelism-inference-scaling/)：V3.2 的 selected core attention 为 $O(Lk)$，lightning indexer 仍扫描每个 query 的全局历史。SGLang 的 DSA Prefill CP 将 query rows 分给多个 ranks，通过全局 index K / latent KV 可见性、zigzag / interleave 和通信重叠分摊长 Prefill 的 indexer 与 selected attention。
