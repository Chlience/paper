...
- 与 [Grape](/papers/2026-07-13-grape-micro-task-agentic-workflow-serving/)：Sarathi 把一个请求的 prefill 切成 chunk，与 decode 组成 decode-maximal batch；Grape 将相同执行原语扩展到相依 workflow task，让上游 decode chunk 直接成为下游增量 prefill 的输入，并在 SLO 约束下调度这些微任务。两者分别提供 chunk primitive 与跨 task dataflow lowering。
...
- 与 [HybridFlow](/papers/2409.19256-hybridflow-rlhf-framework/)、[verl](/papers/2026-06-16-verl-rl-optimization-algorithms/) 和 [slime](/papers/2026-06-17-slime-rl-scaling-framework/)：三者编排 actor、rollout、reward 和 policy update 的训练-生成 dataflow；Sarathi 处理 rollout / inference engine 内部的 prefill/decode scheduling。两层组合时需要同时记录吞吐、长尾、batch composition 与 logprob consistency。
...
- 跨论文关系定位：记录 Sarathi、Chunked Prefill 与 Prefill/Decode Scheduling，并连接 LLM Serving、FlashAttention、Parrot/Span Query、Vortex、Inference Determinism 与 TIM。
