...
ReOPD 把教师 RL 轨迹拆成按轮次采样的 replayed prefixes，让学生只在选定的当前步生成动作并接受 teacher reverse-KL token supervision，从而在学生训练阶段实现零工具调用；Qwen3 数学与搜索实验显示其单次 rollout 至少快于在线 OPD 4 倍且平均准确率持平或更高，可信边界包括必须具备带 observation 的教师轨迹池、教师支持度只是可靠性的代理、固定 $\kappa=0.6$ 尚无自适应保证，以及当前仓库缺少核心 prefix-pool 构造脚本。
...
- Multi-turn OPD 可以拆成 prefix source、current-action policy、teacher target 三个独立设计轴。系统设计时应明确每一轴的分布来源。
- 已保存的 teacher RL trajectories 具有二次价值：既是 teacher optimization 的 rollout，也能成为多个 student 的离线 distillation substrate。
- “on-policy”需要按粒度标注。ReOPD 的当前 action tokens 是 student-on-policy，多轮 prefix 是 teacher-forced；这一区分决定能否推断部署 occupancy coverage。
- 环境成本高时，可把 live environment 收缩到 teacher data-production 阶段，再以可版本化的 trajectory pool 支撑 student iteration。
- 更稳健的后续版本应直接估计 per-prefix support overlap 或 teacher uncertainty，并让 $\kappa$ 随 teacher—student gap、任务和 step 自适应。
