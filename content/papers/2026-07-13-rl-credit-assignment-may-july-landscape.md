...
- Analysis modules: experiment, system, theory, survey
- Key figure decision: omit
- Key figure rationale: 这份时间窗综述的首要贡献来自跨论文时间线、统一比较轴和证据边界，没有一张单一来源图能够同时承载这些关系；公开页面改用结构化研究窗口标记和比较表。
- Review status: page-type=not-applicable; match-confidence=high; observed-at=2026-07-22; venue-status=composite synthesis across arXiv, ACL 2026, OpenReview and archived paper notes
...
- Search window: 2026-05-01 至 2026-07-22 15:08 CST
- Published / updated: 2026-07-22
- Current version read: 检索截止时可访问的 arXiv abstract / HTML、ACL Anthology proceedings page、OpenReview metadata / paper page，CCPO v1 / v3 / v5 版本对照，以及 SPIRAL、LLM-as-a-Verifier、TRACE 等已存档全文笔记
- Accessed: 2026-07-22
...
| 层级 | 查询与来源 | 用途 |
| --- | --- | --- |
| 精确词检索 | arXiv API：`all:"credit assignment" AND submittedDate:[202605010000 TO 202607132359]` 的 212 条原始结果，加上 2026-07-14 至 2026-07-22 的增量复查 | 覆盖显式使用 credit assignment 表述的论文，并保留可复核的首轮搜索底稿 |
...
时间统一按首次公开日期排序。ACL 2026 的 [Fine-Mem](https://aclanthology.org/2026.acl-long.900/)、[CriticSearch](https://aclanthology.org/2026.findings-acl.596/)、[ELPO](https://aclanthology.org/2026.acl-long.504/) 和 [CW-GRPO](https://aclanthology.org/2026.acl-long.1462/) 在 7 月获得正式论文集页面，其中若干方法的 preprint 早于 5 月，因此单独作为正式发表更新。CCPO 同样在窗口前首次公开；[v1](https://arxiv.org/html/2603.21563v1) 尚未包含 SEPO，5 月 29 日的 [v3](https://arxiv.org/html/2603.21563v3) 加入 verifier-anchored 自评与互评分配器，6 月的 [v4](https://arxiv.org/html/2603.21563v4) 与 [v5](https://arxiv.org/html/2603.21563v5) 将其定名并完善为 SEPO。这组版本变化作为窗口内实质修订单列，不计入“5 月以后首次提出”的时间线。
...
- [SPIRAL](/papers/2606.23595-spiral-learning-search-aggregate/) 与 [LLM-as-a-Verifier](/papers/2607.05391-llm-as-a-verifier/) 共享 Chelsea Finn，连接集合级搜索—聚合训练与冻结 verifier 的连续进展信号。
...
- 与已存档论文的直接关系包括 [Credit Assignment Survey](/papers/2604.09459-credit-assignment-reasoning-agentic-llm-rl/)、[SRPO](/papers/2605.25507-credit-assignment-resets-language-model-reasoning/)、[VIMPO](/papers/2606.20008-vimpo-value-implicit-policy-optimization-llms/)、[SPIRAL](/papers/2606.23595-spiral-learning-search-aggregate/)、[ECHO](/papers/2606.31650-echo-selective-turn-memory-agentic-rl/)、[CompactionRL](/papers/2607.05378-compactionrl-context-compaction-agent-rl/)、[LLM-as-a-Verifier](/papers/2607.05391-llm-as-a-verifier/)、[SAO](/papers/2607.07508-sao-single-rollout-asynchronous-agentic-rl/) 与 [TRACE](/papers/2607.13988-trace-turn-level-reward-assignment/)。

## 一句话结论

2026 年 5—7 月的 RL 信用分配研究围绕 token、segment、turn、trajectory set、memory operation 和 workflow role 六种 credit unit 形成可比较的方法谱系；compact agent 进一步把压缩后状态可比性列为 estimator 条件，而 SPIRAL、SEPO 与 TRACE 又把生成策略、聚合或互评策略、冻结参考策略之间的拓扑关系带入信用来源分析。
...
2. 每种方法把 credit 分给什么单元，使用什么识别信号，通过什么算子进入 policy loss，以及生成者与评价者是否共享参数并共同更新。
...
- 纳入范围：2026 年 5 月 1 日至 7 月 22 日首次公开、直接改变局部信用映射或其 estimator 的 LLM reasoning / agentic RL 工作，并单列窗口内正式发表和实质版本修订。
- 系统条件：跨论文模型、环境、rollout 数、工具接口和训练基础设施不统一，只比较机制条件与证据边界。
- 指标定义：task return、attribution fidelity、credit sign accuracy、state comparability、gradient concentration 和 total compute 分开记录。
- 成本归因：额外 rollout、teacher / reference forward、judge call、critic FLOPs、graph construction 和环境重放分别计入，不把更多计算直接解释为更强归因。
- 假设：局部识别信号需要与真实边际贡献或未来 return 保持足够相关，且比较样本满足各 estimator 声明的条件可比性。
- 适用域：结论覆盖 LLM reasoning、search、tool use、memory 与 context compaction；经典 MARL 仅在其直接改变 LLM 多角色信用时进入对照。
...
\text{CA method}
=
(\text{credit unit},\ \text{identification signal},\ \text{assignment operator},\ \text{comparability condition},\ \text{policy topology}).
$$
...
| Policy topology | 单 actor、共享参数多角色、多个可训练策略、actor + 冻结 reference、actor + 独立 critic | 梯度干扰、评价目标漂移、互评串通、额外推理成本 |
...
| [SPIRAL](/papers/2606.23595-spiral-learning-search-aggregate/), 06-22 | trajectory set / search trace | 聚合策略在随机候选集合上的成功率经 participation average 回传给搜索轨迹，同集合内 reward 更新聚合轨迹 | 主实验由同一 LoRA 扮演 search / aggregate；集合参与相关仍含共现成员噪声 |
...
6 月的变化集中在 estimator。Bayesian ratio、自蒸馏 divergence、attention flow、graph TD、action-conditioned peer 与 counterfactual sibling 都试图在额外 rollout、critic bias 和近似假设之间重新取舍。SPIRAL 进一步让下游聚合结果决定上游搜索轨迹的集合级信用。memory 路线同时分成三支：对 compact state 的质量打分、对 memory write 做局部反事实、沿 source provenance 路由最终 advantage。

窗口外首发、窗口内经 v3—v5 实质修订的 [CCPO / SEPO](https://arxiv.org/html/2603.21563v5) 提供了另一种多策略结构：Thinker 与 Solver 都参与正常任务生成，SEPO 再让两个角色提供受限的自评与互评，由外部 verifier 锚定并重新分配角色奖励。它最接近“可训练任务策略也参与信用分配”，当前信用粒度仍停留在角色与轨迹层，实验集中于双角色数学推理。

### 5. 2026 年 7 月：校准、参考策略与单 rollout critic
...
| [LLM-as-a-Verifier](/papers/2607.05391-llm-as-a-verifier/), 07-06 | prefix / process step | 冻结 LLM 的评分 token 概率期望构成连续进展分数，并作为 dense RL shaping signal | 当前 RL 证据是单轮、小规模 proof of concept；pair context、校准与验证成本仍影响解释 |
...
| [TRACE](/papers/2607.13988-trace-turn-level-reward-assignment/), 07-15 | tool turn | 冻结初始化参考模型测量每个工具边界后标准答案的可预测性变化，用有限步 TD shaping 混入终局优势 | 需要已知短答案和额外 reference scoring；相邻前缀进展仍弱于动作反事实 |

截至 7 月 22 日，公开工作更关注 credit 的可靠性和参照系：TACO 限制低概率 token 获得过强正向更新，PivoARL 与 DASH 定位错误或漂移，SAO 与 CompactionRL 用 critic 支持单 rollout 和分段轨迹，LLM-as-a-Verifier 与 TRACE 则让冻结模型沿 prefix 产生连续进展信号。该阶段仍缺少统一的 attribution benchmark。
...
5. SPIRAL、SEPO、LLM-as-a-Verifier 与 TRACE 说明 policy topology 已经成为独立变量：生成、聚合、互评、reference scoring 和 critic fitting 对应不同信用语义。
6. 7 月的方法把重点推进到正向 credit 校准、pivotal error localization、single-rollout critic、冻结参考策略进展信号与跨 compaction boundary 的 temporal credit。
7. 当前方法仍未同时提供低成本、严格因果、双向 credit、递归 provenance、可靠 value calibration 和跨任务验证。
...
### 结果 3：7 月新工作把校准、reference scoring 和 temporal estimator 推到训练主路径

- 设置：TACO、PivoARL、DASH、SAO、CompactionRL、LLM-as-a-Verifier 与 TRACE 分别处理正向 tail credit、pivotal error、segment drift、single-rollout GAE、compaction segment、连续 verifier progress 与冻结 reference turn credit。
...
- 结果：七篇论文均报告局部机制带来正向效果；证据分散于 reasoning、coding、robotics 与 search-agent benchmarks，统计和 compute matching 程度不同。
- 证据定位：[TACO](https://arxiv.org/abs/2607.07976)、[PivoARL](https://arxiv.org/abs/2607.03702)、[DASH](https://arxiv.org/abs/2607.00482)、[SAO](/papers/2607.07508-sao-single-rollout-asynchronous-agentic-rl/)、[CompactionRL](/papers/2607.05378-compactionrl-context-compaction-agent-rl/)、[LLM-as-a-Verifier](/papers/2607.05391-llm-as-a-verifier/) 与 [TRACE](/papers/2607.13988-trace-turn-level-reward-assignment/)。
...
- arXiv API 的 212 条首轮精确检索结果提供明确搜索底稿；7 月 14—22 日增量复查和机制查询补入 TRACE，并覆盖没有使用精确词组的核心 compact 方法。
- Memory-R2、HiMPO、ECHO 与 CompactionRL 的官方正文对 credit source 和 update target 有清晰公式或流程，可做机制层比较。
- SPIRAL 与 TRACE 已完成本地全文、公式和证据边界审计，分别支持集合级聚合信用与冻结参考模型 turn shaping 的机制定位。
- ACL Anthology 提供 Fine-Mem、CriticSearch、ELPO 与 CW-GRPO 的正式论文集元数据和摘要。

### 中等强度证据

- 5—7 月方法沿边界定位、counterfactual、critic、provenance、policy topology 和 calibration 演进的趋势由多篇独立论文支持，但发表密度也受 arXiv 命名与检索词影响。
...
| 拓扑 | 正常任务生成 | 信用来源 | 当前边界 |
| --- | --- | --- | --- |
| SEPO | Thinker 与 Solver 两个可训练角色依次生成推理和答案 | 两个角色自评、互评，外部 verifier 约束奖励重分配 | 最接近多个任务策略共同归因；粒度仍是角色 / 轨迹，存在互评校准与串通风险 |
| SPIRAL | search 与 aggregation 角色生成候选和最终答案 | aggregation outcome 经集合参与均值回传 search traces | 主实验共享同一 LoRA；credit 是集合参与关联，没有显式反事实移除 |
| TRACE / VIMPO | 当前 actor 生成任务轨迹 | 冻结 reference 提供 turn progress 或 token implicit value | reference 不更新，也不承担正常任务角色；信用目标相对稳定 |
| 独立 critic / verifier | actor 生成，critic 或 verifier 估值 | learned value、judge score 或 progress reward | 评价器与行为策略解耦，仍需处理 bootstrap、proxy drift 和额外计算 |

严格口径下仍缺少一套成熟方案：多个独立、同时学习的策略轮换生成与评价角色，在 turn 或 token 层产生双向信用，并用外部结果约束互评总量。可检验的设计需要让 peer score 只重新分配总奖励，使用滞后 evaluator 或交叉拟合降低同批自评偏差，再用 leave-one-role-out rollout 校准互评分数与真实边际贡献的一致性。
...
- `cross-policy peer credit + external outcome anchor`：让任务策略互评分配角色或 turn credit，并限制互评只能重分配外部结果。

## 主要启发

- Credit assignment 方法应同时声明 credit unit、识别信号、分配算子、state comparability 条件和 policy topology。
...
- 共享参数多角色、多个可训练策略和冻结 reference 产生不同的信用语义；论文应明确哪些模型参与任务生成、哪些模型输出评价、哪些参数接受更新。
- 后续论文应把 attribution fidelity、task performance 与 total compute 三类结果同时报告。

## 局限

1. 检索截止到 2026-07-22 15:08 CST，后续新版本和当天晚些时候发布的条目未覆盖。
...
- 与 [Credit Assignment Survey](/papers/2604.09459-credit-assignment-reasoning-agentic-llm-rl/)：survey 截止 early 2026，本综合补充 2026-05-01 至 2026-07-22 的方法增量，并增加 compact state comparability 与 policy topology 两条判断轴。
...
- 与 [SPIRAL](/papers/2606.23595-spiral-learning-search-aggregate/)、[LLM-as-a-Verifier](/papers/2607.05391-llm-as-a-verifier/) 和 [TRACE](/papers/2607.13988-trace-turn-level-reward-assignment/)：三篇分别用下游聚合结果、冻结 verifier score 和冻结 reference answer likelihood 形成信用，补齐 set-level、process-level 与 turn-level 的策略拓扑差异。
