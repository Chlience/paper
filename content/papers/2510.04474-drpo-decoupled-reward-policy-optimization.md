...
- Review status: page-type=official-review; match-confidence=high; observed-at=2026-07-29; venue-status=ICLR 2026 Poster
- License: CC BY 4.0
...
1. 对提示 $q$，旧采样策略 $\pi_{\mathrm{old}}$ 生成一组在线采样输出（rollout）；二元验证器（verifier）给出正确性标签并记录长度，形成正确集合 $S_q^+$ 和错误集合 $S_q^-$。
2. 当前策略 $\pi_\theta$ 为每条输出计算词元平均对数似然，得到序列分数 $s_\theta(o,q)$，并把分数传入对应的正样本或负样本分支。
3. 正样本分支依据内层 KL 正则化的闭式解，在 $S_q^+$ 内计算与 $\exp(r_l(o)/\lambda)$ 成比例的经验权重 $\hat\omega_i$；较短的正确输出获得较大正系数，较长的正确输出获得较小且仍为正的系数。
4. 正样本分支最大化加权序列分数；负样本分支沿用 DisCO 的 log-sum-exp 项，对高分错误输出施加更强的抑制信号。两个分支组成最终的序列级判别训练信号。
5. 外层 KL 约束限制 $\pi_\theta$ 相对 $\pi_{\mathrm{old}}$ 的单次更新幅度，优化器据此更新当前策略。

关键对象的角色如下：

- $\pi_{\mathrm{old}}$ 是冻结的旧采样策略，负责生成当前训练组；$\pi_{\mathrm{old}}^+$ 是它在正确输出上的条件分布，也是内层分布优化的参照。
- $P_q^*$ 是长度奖励与内层 KL 共同定义的理想正确样本分布，用于导出权重；它不等于待更新策略。
- $\pi_\theta$ 是实际计算序列分数并接受梯度更新的当前策略。
- $\lambda$ 是内层 KL 正则化系数，同时控制长度倾斜强度；$\lambda$ 越大，长度偏好越弱，$\lambda\to\infty$ 时 DRPO 回到 DisCO。

DisCO 提供正负样本划分、序列分数、困难错误样本聚合和外层策略更新约束；DRPO 在其正样本分支中加入正确输出内部的长度重加权。内层 $D_{\mathrm{KL}}(P\Vert\pi_{\mathrm{old}}^+)$ 构造 $P_q^*$ 并导出长度权重，外层 $D_{\mathrm{KL}}(\pi_{\mathrm{old}}\Vert\pi_\theta)$ 直接约束实际策略更新。
...
若 group reward 的标准差大于 0、错误 response 的 reward 均为 0，正确集合 $S_q^+$ 内的组合 reward 为 $u_1,\ldots,u_m$，那么正确 response $i$ 发生符号翻转的精确条件是：
...
论文 Figure 1 构造三条正确、三条错误 response。纯正确性 reward 为 $[1,1,1,0,0,0]$ 时，三条正确 response 都得到正 advantage；加入长度惩罚后，组合 reward 变成 $[0.73,0.6,0.2,0,0,0]$，图中把第三条较长但正确 response 的 advantage 标为约 $-0.17$。图示 reward 和 advantage 已经过小数舍入，显示值无法精确复算该小数，符号结论不受影响。
...
它是对旧 policy 正样本分布的 exponential tilt（指数倾斜）。对两条正确 response $o_a$ 与 $o_b$，目标分布中的相对概率比满足：

$$
\frac{P_q^*(o_a)}{P_q^*(o_b)}
=
\frac{\pi_{\mathrm{old}}^+(o_a\mid q)}
{\pi_{\mathrm{old}}^+(o_b\mid q)}
\exp\left(
\frac{r_l(o_a)-r_l(o_b)}{\lambda}
\right).
$$

长度 reward 越高的正确 response 获得越有利的相对概率比乘数；绝对概率及其排序仍同时取决于 $\pi_{\mathrm{old}}^+$ 的基准概率比，支持集也受该分布限制。$\lambda$ 越小，长度偏好越尖锐；$\lambda\to\infty$ 时分布层面的重要性比率趋于 1，$P_q^*$ 回到 $\pi_{\mathrm{old}}^+$，DRPO 退化为 DisCO。
...
对于线性长度 reward，$\exp(1/\lambda)$ 会在归一化中抵消，因此相对权重可以改写为：

$$
\omega(o\mid q)
\propto
\exp\left(-\frac{|o|}{C\lambda}\right).
$$

$C\lambda$ 是控制长度偏好变化速度的有效尺度：两条正确 response 相差 $C\lambda$ 个 token 时，较长 response 的相对重加权因子约乘以 $e^{-1}$。这是对论文公式的本地重写，用来解释 $\lambda$ 如何映射到实际长度差异。
...
这里同时出现两个作用不同的 KL。内层 $D_{\mathrm{KL}}(P\Vert\pi_{\mathrm{old}}^+)$ 产生正样本目标分布的指数倾斜；DisCO 外层的 $D_{\mathrm{KL}}(\pi_{\mathrm{old}}\Vert\pi_\theta)$ 约束实际 policy update。闭式解只对应理想分布 $P_q^*$，没有给出实际策略 $\pi_\theta$ 的闭式解。
...
理论重要性比率满足 $\mathbb E_{\pi_{\mathrm{old}}^+}[\omega]=1$，且在 $\lambda\to\infty$ 时逐点趋于 1；有限 group 的归一化系数满足 $\sum_{i\in S_q^+}\hat\omega_i=1$，并在同一极限下趋于 $1/|S_q^+|$。两种口径都让正样本项回到 DisCO，数值归一化含义不同。
...
当某组只有一个正样本时，$\hat\omega=1$，当前组退化为 DisCO 的正样本项，不提供长度间排序。全对 group 即使包含多条不同长度的正确 response，官方实现也会把整组判别项清零。DRPO 的长度信号因而同时受到题目 solve rate、group size 和过滤规则影响。

对每题采样 $G=8$ 条 response，若给定问题的单次成功概率为 $p$，并用独立采样近似 rollout，那么 DRPO 特有的长度排序要求正确样本数 $K$ 满足 $2\leq K\leq7$，其覆盖概率为：

$$
\Pr(2\leq K\leq7)
=
1-(1-p)^8-8p(1-p)^7-p^8.
$$

例如 $p=0.1$、$0.5$、$0.99$ 时，覆盖率分别约为 $18.7\%$、$96.1\%$、$7.7\%$。这是基于实现条件的本地推导，用来说明长度学习集中在中等 solve rate 区域；论文没有报告这些 group 类型的真实占比或训练期变化。

虽然训练配置保留 `adv_estimator=grpo`，`compute_policy_loss_drpo` 的主损失没有读取 GRPO advantage，也没有使用 PPO importance ratio 或 clipping。实现中的 DRPO 主目标就是 DisCO 的正负序列判别项，加上正样本内部的长度 softmax 权重。
...
- 指标：各数据集 Pass@1、平均长度与 AES（Accuracy Efficiency Score，准确率—效率分数）。记 $\Delta\mathrm{Length}$ 为相对长度缩减、$\Delta\mathrm{Acc}$ 为相对准确率变化；准确率下降时，$\mathrm{AES}=\Delta\mathrm{Length}-10|\Delta\mathrm{Acc}|$，因此正 AES 仍允许少量准确率下降。准确率上升时，公式使用 3 倍增益奖励。每种方法又使用自己的无长度版本作为 reference，AES 更适合观察方法内部的边际交易。
...
1. 首要贡献依赖 binary correctness partition；开放式生成、部分正确、过程 reward 与多维 preference 只获得形式层面的扩展说明。指数倾斜也不会创造旧 policy 尚未覆盖的正确轨迹；旧分布强烈偏向长轨迹时，有限 $\lambda$ 不保证短轨迹拥有更高绝对概率。
...
- 与 DisCO（[arXiv:2505.12366](https://arxiv.org/abs/2505.12366)）的关系最直接：Gang Li、Ming Lin、Tianbao Yang 三位作者重叠；DRPO 保留 DisCO 的正负判别 score、hard-negative log-sum-exp 和 KL constraint，把长度 reward 变成相对于旧正样本条件分布的指数重要性比率。DRPO 的 novelty 应理解为 DisCO 上的多 reward 扩展和 efficient-reasoning 实证。
