...
- 与 [DeepSeek-R1](/papers/2501.12948-deepseek-r1-rl-reasoning/)：R1-Zero 的 “aha moment” 容易被解释为人类式顿悟；本文提供方法论边界，应将其写成特定训练与采样条件下出现的可观测 reasoning behavior。
- 与 [SocioHack](/papers/2606.04075-llms-hack-rewards-and-society/)：SocioHack 研究 RL 模型在制度规则中形成漏洞发现行为；本文提醒将行为、优化机制和意图归因分层，避免把策略结果直接解释为稳定的人类式属性。
- 与 [Batch-Invariant Inference](/papers/2025-09-10-defeating-nondeterminism-llm-inference/) 和 [tool-calling RL](/papers/2606.00135-agentic-tool-calling-rl-training/)：两者分别说明 serving 数值路径和 evaluation harness 会改变观测行为，为本文的 interface / substrate sensitivity 提供工程侧实例。
