...
- 与 [2607.07508 SAO](/papers/2607.07508-sao-single-rollout-asynchronous-agentic-rl/) 的关系：两者都处理 long-horizon token-level advantage variance。OTB 用同 prompt group samples 估计 variance-minimizing baseline，`rollout.n=1` 时会退化；SAO 恢复一份显式 learned critic，使每 prompt 单 rollout 仍能获得 state-dependent baseline，同时增加 value pretraining、模型内存和 critic updates。
