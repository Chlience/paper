...
给定任务 $x$、两条轨迹 $\tau_i,\tau_j$ 和 criterion $c$，验证模型输出两个评分 token。设有序 token 集为：

$$
\mathcal{V}_{\text{score}}=\{v_1,\ldots,v_G\},
$$

映射 $\phi(v_g)$ 把 token 映射到标量等级。论文定义：

$$
R(x,\tau)=\frac{1}{CK}\sum_{c=1}^{C}\sum_{k=1}^{K}
\sum_{g=1}^{G}p_\theta(v_g\mid x,c,\tau)\phi(v_g).
$$

分数再线性归一化到 $[0,1]$。两条轨迹的软偏好写为：

$$
P(\tau_i\succ\tau_j\mid x)=\sigma\bigl(R_i-R_j\bigr).
$$

这一路径依赖四个关键假设：

1. score token 的顺序映射能够表达任务质量。
2. API 提供的 logprob 足以近似相关 score-token 概率质量。
3. 同一轨迹在不同 opponent 与 A/B 位置下的评分具有足够稳定性。
4. criteria 平均和重复平均可以降低方差，同时不会掩盖少数关键失败条件。
