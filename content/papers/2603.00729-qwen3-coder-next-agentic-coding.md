...
Qwen3-Coder-Next 的核心判断是：在编码智能体上，80B total / 3B active 的 Qwen3-Next hybrid attention + MoE 基座，可以通过大规模 executable environment、repo 级 mid-training、多模板 tool calling、SWE 专家 RL 和 expert distillation，逼近更大 active compute 模型的 agentic coding 能力。证据最强的部分是公开权重、公开 model card、多 benchmark 结果和任务合成规模；需要审慎看待的部分是内部 MegaFlow 基础设施、训练成本、数据生成细节和 reward hacking 防护的完整可复现性。
...
- Venue status: arXiv / CoRR technical report；未找到可可靠匹配的正式投稿 forum。
- Public reviews: 0 条可公开核验的 official review。
- Ratings / confidence: 未发现公开评分。
- Reviewer consensus: 无可吸收的正式审稿材料。
- Main criticisms: 无可吸收的正式审稿材料。
- Author response: 未发现公开 rebuttal。
- 对可信度的影响: 当前可信度主要来自 arXiv 技术报告、公开模型权重、官方仓库和 model card；缺少第三方审稿意见和公开复现实验。
...
- 新增后应更新的索引 cluster：Cluster AZ（Qwen2.5/Qwen3/Qwen3-Coder）、Cluster BB（Self-Improving Agents / Harness / Experience Infrastructure）、Cluster BD（LLM-in-Sandbox / Sandbox-Agent Interaction）。

## Reference Intake Brief

### Target

- Intended target system: 新增论文笔记与 `content/utility/papers-index.md`。
- Existing related assets: `content/utility/papers-index.md`；`content/papers/2412.15115-qwen2-5-technical-report.md`；`content/papers/2505.09388-qwen3-technical-report.md`；`content/papers/2601.16206-computer-environments-agentic-intelligence.md`；`content/papers/2602.13692-thunderagent-program-aware-agentic-inference.md`。
- Proposed form: 新建独立 Markdown 文档，并更新索引 cluster 与作者档案。

### Reusable Elements

1. Qwen3-Coder-Next 作为 80B total / 3B active hybrid attention + MoE coding agent model 的模型卡片信息。
2. executable task synthesis / Docker env / MegaFlow / Argo workflow 作为 agentic training infrastructure 案例。
3. tool-template diversity、`qwen3_coder` XML parser、reward hacking blocker 和 best-fit packing 作为 coding-agent 训练与部署经验。

### Risks

- Copyright/over-copying: 只保留关键数字、结构和结论摘要，避免复制论文原文长段落。
- Unsourced or unverifiable claims: 对 MegaFlow、reward hacking blocker、training cost 和数据过滤保留“未公开 / 未完全披露”的边界。
- Tone/brand mismatch: 采用技术档案语气，避免 marketing-style 宣称。
- Safety/compliance issues: 记录 reward hacking 与 cybersecurity 评测风险，不沉淀可直接滥用的操作细节。
- Overlap with existing assets: 与 Qwen2.5 / Qwen3 关系写入索引，避免把 foundation model report 与 coding-agent report 混为一篇。

### Skipped

| Material | Reason |
| --- | --- |
| 公开 reviewer comments | 按标题、作者和 arXiv id 未找到可可靠匹配的正式审稿 forum。 |
| MegaFlow 代码与环境构建 agent 代码 | 论文和官方仓库未公开完整实现。 |
| reward hacking blocker 完整规则 | 报告只描述启发式方向和人工检查结论。 |
| 训练硬件 / GPU hours / 成本 | 报告未披露。 |

### Recommendation

Decision: merge

Why: 该报告补齐本地 Qwen-Coder / agentic coding 训练线的关键节点，且与 LLM-in-Sandbox、ThunderAgent、RollArt、slime、GLM-5、ImpossibleBench 等已有归档形成明确关系。
