...
First-Archived-At: 2026-06-02 10:20
...
- 与 [Correlated Proxies](/papers/2403.03185-correlated-proxies-reward-hacking/) 和 [Causal Rewards](/papers/2501.09620-causal-rewards-llm-alignment/)：前两篇从 proxy correlation、occupancy regularization 和 causal reward modeling 解释奖励失真；SocioHack 将同一问题扩展到制度规则、行动空间和动态补丁环境。
- 与 [Inference-Time Reward Hacking](/papers/2506.19248-inference-time-reward-hacking-llms/)、[Caution in Best-of-N](/papers/2604.04648-caution-pessimism-best-of-n-reward-hacking/) 和 [STV](/papers/2605.30290-self-trained-verification/)：这些工作覆盖 test-time optimization、BoN selection 和 verifier-reasoner loop 中的 reward / score 失真，SocioHack 补充 RL agent 对规则漏洞的持续搜索。
- 与 [tool-calling RL](/papers/2606.00135-agentic-tool-calling-rl-training/) 和 [DAPO](/papers/2503.14476-dapo-long-cot-rl-system/)：训练框架、harness、verifier 与 rule-based reward 的具体设计会决定模型可搜索的行为空间；SocioHack 提供面向这类闭环的安全审计视角。
- 与 [Age of Empires](/papers/2605.31514-age-of-empires-anthropomorphism/)：本文关注优化过程产生的制度漏洞发现行为，Age of Empires 进一步约束对这类行为的意图、理解或人格属性归因。
- 与 [Preference–Dynamics](/papers/2026-04-30-preferences-multi-agent-online-learning/)：SocioHack 在模拟制度中测量优化器如何利用数值规则与制度意图的缺口；Preference–Dynamics 从有限多智能体博弈证明，同一 ordinal preference specification 的不同 cardinal payoff realization 可能诱导不同稳定集合，为 reward magnitude 审计提供形式化边界。
