...
- Canonical source: https://thomas-yang.github.io/#-selected-publications
...
- Responsible organization: 公开可确认关联机构为 Beihang University；论文 PDF 本身未披露机构
- PDF: 用户提供的 12 页匿名稿，本地文件名 `main.pdf`，SHA-256 `90b901377ad8ccedb1d8d824b4eb32e356b001023b01b1d5931a3a8049cac44f`
- Code/Project: 未发现公开代码仓库或项目页
- OpenReview / Review page: 未发现可公开访问的正式审稿页
- Venue page: https://sc26.supercomputing.org/program/papers/
- Submitted: 未披露
- Published / updated: 2026-07，Hailong Yang 主页公布 Grape 被 SC 2026 接收
- Current version read: 12 页匿名 PDF，未标注版本号；PDF metadata 的 creation date 为 2026-04-09，文件哈希见上
...
公开关系证据来自 Hailong Yang 的主页：2026 年 7 月新闻把 Grape 的 SC 接收与 Siqi Wang 关联，Selected Publications 同时列出论文全名；该主页也确认 Hailong Yang 为北航计算机学院教授。同机构关系可以确认，匿名稿无法恢复作者顺序、共同一作、通讯作者、完整机构列表或跨机构桥接。

## 一句话结论

Grape 面向预声明的多阶段 LLM task DAG (Directed Acyclic Graph，有向无环图)，把静态 prompt、流式上游输出和 decode 拆成微任务，让下游已知 prefix 与上游逐步生成的中间结果在上游 decode 期间持续完成增量 prefill，再用服务级目标约束的 batch 构造和关键路径感知 Key-Value Cache (KV Cache) 抢占控制尾延迟。

本地评价：最有价值的新增机制是跨 task 增量 prefill。它把 Parrot 式应用数据流继续下降到一次 LLM 调用内部的 prefill / decode 边界，并把 Sarathi 式 chunked prefill 扩展到依赖调用之间。总 prefill token workload 大体保留，收益来自执行时间重排、mixed prefill/decode batching、提前构建下游 KV 和压低 task-switch token gap。相对同 vLLM 基座且最接近的 `vLLM-opt`，完整 Grape 的平均 agentic inter-token latency 和吞吐收益分别为 1.15 倍和 1.16 倍，P95 agentic inter-token latency 收益达到 3.80 倍；论文只单独消融了 memory optimization，因此这组数字支持完整系统，无法继续拆分 incremental prefill、scheduler 与 memory manager 的独立贡献。
...
2. TaskFlow、微任务图、SLO (Service-Level Objective，服务级目标) 约束调度和 KV Cache 抢占如何组成一条完整执行路径。
3. 论文的尾延迟、吞吐和 SLO 证据相对最接近的同基座 baseline 能支持多大的结论。

判断边界：

- 论文 PDF 匿名且没有公开 artifact；作者、机构、venue 和实现状态分别按本地 PDF 与公开作者主页记录。
- SC 2026 接收状态由作者主页直接支持，尚未由会议 proceedings 页面复核。
- TaskFlow 的控制拓扑在执行前声明；`dynamic node` 表示 token 内容或执行次数在运行时确定，论文没有覆盖 condition、loop、retry、dynamic fan-out 或运行时 task 创建。
...
论文研究的 agentic workflow 是预先声明控制拓扑的多阶段 LLM task DAG。上游 agent 先 decode 中间结果，下游 agent 再把该结果插入自己的 prompt，完成一次完整 prefill 后开始 decode。通用 serving engine 通常把每次调用视为原子请求，因此依赖调用按 task 串行推进。
...
Grape 的切入点由这些系统自然顺承：应用层已经知道依赖图，LLM engine 已经具备 chunked prefill 和 continuous batching，因此可以把 prompt 的静态段、动态 `IntermediateVar` 段和 decode 分别暴露给同一个调度器。这里的 overlap 指调度时间线上的交错推进，以及一次 forward 中的 mixed prefill/decode batch；它不要求两个 GPU kernel 并发运行。
...
令 $\mathcal E(K,X,o)$ 表示从已有 KV state $K$ 出发，在起始位置 $o$ 追加 token block $X$ 并完成 causal prefill。增量执行可以写成

$$
\begin{aligned}
K_0 &= \mathcal E(\varnothing, S_B \Vert U_B, 0), \\
o_i &= |S_B \Vert U_B| + \sum_{j<i}|C_j|, \\
K_i &= \mathcal E(K_{i-1}, C_i, o_i), \qquad i=1,\ldots,m.
\end{aligned}
$$

在相同 token 序列、位置编码、mask、模型参数和理想算术下，$K_m$ 与对完整 $P_B$ 执行一次 monolithic prefill 得到的 KV state 对应。每个新 token 只读取此前 prefix，因而 chunk 边界不会改变语义依赖。实际 GPU kernel 可能因 batch composition、reduction order 和精度改变末位数值；若 logits 经过采样，微小差异仍可能放大成不同输出轨迹。
...
lowering 阶段把每个 task 分成 prefill 与 decode，再以 `IntermediateVar` 边界切分 prefill。固定 system prompt 和 `InputVar` 进入 static node；依赖上游输出的 prompt 段和 decode 进入 dynamic node。这里的 dynamic 表示 token 数量或迭代次数到运行时才确定，TaskFlow 控制拓扑仍然预先声明。task 内严格顺序用 strong edge 表示，跨 task 的 token streaming 用带权 partial edge 表示；edge weight 就是每次传输和 prefill 的 token chunk 大小。较小 chunk 更早传播依赖并增加调度与 operator 开销，较大 chunk 提高执行效率并延后 overlap，Grape 通过离线 operator profiling 选择权重。证据定位：Section III-B/C，Figure 6-7，PDF pp.4-5。
...
5. 相对同 vLLM 基座且最接近的 `vLLM-opt`，Grape 的主要实证收益集中在 P95 agentic inter-token latency，同时保持约 15%-16% 的平均 agentic inter-token latency 与吞吐增益。

## 关键实验/定理

### 结果 1：相对最接近的同基座 baseline，尾延迟收益最大

- 设置：Llama3-8B 运行在 1 张 A100 40GB；Qwen3-14B 运行在 2 张 PCIe 互联 A100 40GB；工作负载为 code、summary 和 search。
- Baseline：Parrot、vLLM，以及把 Parrot 优化移植到同一 vLLM 基座的 `vLLM-opt`。论文只说明 `vLLM-opt` incorporates the optimizations from Parrot，没有列出具体移植项。
- 指标：average 与 P95 agentic inter-token latency，单位为 ms/token；中间 task token、最终输出 token 和跨 task 输出空档位于同一时间序列。不同 workload 使用不同 request arrival rate；论文没有报告 final-answer latency 或 workflow makespan 的 P95。
- 结果：相对 Parrot / vLLM / `vLLM-opt`，平均 agentic inter-token latency 加速分别为 2.30 / 1.29 / 1.15 倍，P95 agentic inter-token latency 加速分别为 8.49 / 5.51 / 3.80 倍。
...
- 支持的最窄结论：在论文给定的两种模型、A100 配置、三类 LLM-only workflow 和 arrival-rate 区间内，完整 Grape 显著缓解 agentic token-gap 尾部尖峰，并带来中等幅度平均 agentic inter-token latency 改善。
- 解读：3.80 倍 P95 与 1.15 倍平均值的差异符合缓解 task-switch prefill burst 的机制预期；缺少多次重复、误差条和分位数置信区间，使尾部数字仍需独立复验。
...
| Baseline 是否 tuned | Parrot、vLLM 的具体调参范围未披露；`vLLM-opt` 明确吸收 Parrot 优化，但论文没有列出具体组成或调参范围。 |
| Baseline 是否 compute-matched | 同模型实验看起来使用同一 A100 配置；Parrot 仅在 Llama3 上评估，Qwen3 不具备完整三方可比性。 |
| Baseline 是否 implementation-matched | `vLLM-opt` 是最接近的同 vLLM 基座对照；具体移植项未披露，因此只能确认 engine base 接近。Parrot 使用自建 engine，论文承认其 operator 实现、CUDA Graph 缺失和 memory defragmentation 开销较弱。 |
| Baseline 是否覆盖强替代方案 | 覆盖 vLLM、Parrot 和最接近的同基座版本；未覆盖 Autellix、SGLang program runtime、现代 PD disaggregation 或其他 workflow scheduler。 |
...
- memory optimization 有独立开关对照，平均吞吐收益为 1.06 倍；图维护和优先级开销也有直接运行时测量。

### 中等强度证据

- 三类 workload 覆盖 code、summary 和 search，能够说明机制跨多个静态 workflow 生效；样本和长度分布未披露，代表性仍有限。
- `vLLM-opt` 提供最接近的同 vLLM 基座对照，能够减少 Parrot 底层 engine 差异的影响；论文没有列出它吸收的具体 Parrot 优化，仍保留对照组成不透明的问题。
- latency、throughput、MFU、SLO 和 execution timeline 在完整系统层面方向一致；缺少 incremental prefill、priority policy 与微 batch constraint 的正交消融，因果归因需要保留边界。
...
- Venue status: Hailong Yang 官方主页列为 SC 2026 accepted；SC26 官方 Papers 页面显示 2026-07-01 发通知、2026-08-28 交最终稿，当前尚无可按题名复核的 schedule / proceedings 条目
...
- 平均 agentic inter-token latency、吞吐和 P95 的收益结构显示它主要平滑 task-switch prefill burst，整体算力效率提升处于约 15% 量级。

P95 改善显著大于平均 agentic inter-token latency，可以用稀疏 burst 分布理解。设常规 decode token 的间隔为 $d$，task boundary 或受同 batch 长 prefill 阻塞的 token 间隔为 $b$，后者占比为 $p$ 且 $b\gg d$，则平均值约为：
...
| 收益来源 | 直接作用 | 论文证据 | 本地判断 |
| --- | --- | --- | --- |
| 跨 task incremental prefill | 将下游静态 prompt 和逐步到达的上游输出提前转成 KV，与上游 bandwidth-bound decode 组成 mixed batch；下游 decode 启动前需要补做的 prefill 更少 | 完整 Grape 相对 `vLLM-opt` 的平均 agentic inter-token latency / 吞吐为 1.15 / 1.16 倍，MFU 最高提高 1.15 / 1.16 倍；Figure 19 的 execution timeline 显示 task-boundary bubble 被填充 | 机制上提供主要并行机会；总 prefill token workload 大体保留，收益方向与时间重排、资源互补和等待隐藏一致，独立贡献尚无消融 |
...
6. `vLLM-opt` 是最关键 baseline；相对它的平均 agentic inter-token latency 和吞吐收益约 15%-16%。相对 Parrot 的更大数字受到 Parrot engine 实现限制影响。
7. 单轮执行模型 $F(N_{\text{seq}},N_{\text{batch}})$ 没有报告预测误差，也没有显式建模微任务类型混合、cache residency、kernel path、tensor parallel 和 memory pressure。
8. 关键路径优先级使用拓扑 hop；preemption 的 distance 公式未披露，已披露设计也没有纳入 weighted service time、KV volume、recompute FLOPs 和 time-to-reuse。
...
5. graph-aware KV preemption：以微任务为回收单位，利用 active critical node 的 distance 指标降低阻塞；论文未披露具体公式。
...
Why: Grape 给出了从 workflow variable 到 engine 微任务的完整 lowering，完整系统相对最接近的同 vLLM 基座 baseline 的主要收益集中在 P95 agentic inter-token latency，方向上与减少 task-boundary prefill burst 一致。作者、artifact、正交消融、严格 SLO 保证和工具型 workload 仍有明确证据缺口，适合以带边界的系统论文笔记归档并等待 proceedings / code 更新。
