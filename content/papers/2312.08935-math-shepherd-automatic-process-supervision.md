...
- 与 [LLM-as-a-Verifier](/papers/2607.05391-llm-as-a-verifier/)：Math-Shepherd 用 suffix sampling 与 answer checker 自动生成 step labels，再训练 PRM；LLM-as-a-Verifier 直接从冻结模型的 pairwise logit readout 估计 prefix progress。两者分别代表训练式 process supervision 与 inference-only process proxy。
- 与已有论文的作者关系：与 [2501.12948 DeepSeek-R1](/papers/2501.12948-deepseek-r1-rl-reasoning/) 存在 Peiyi Wang、Runxin Xu、Zhihong Shao、Yu Wu 等作者网络重叠；与 DeepSeek-Math、DeepSeek-Prover、DeepSeek-V 系列有机构和研究线联系。
...
- Proposed form: 新建独立 Markdown 文档 `2312.08935-math-shepherd-automatic-process-supervision.md`，同步索引行和对应论文的关系章节。
