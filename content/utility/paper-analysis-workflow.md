...
- 每位可解析作者完成基础身份核验；默认新建 profile 只覆盖作者顺序前两位、论文明确标注的共同一作和明确通讯作者，普通作者只复用已有 profile 或按用户明确要求建档；触发深入核验且进入建档范围的作者已按字段保存稳定来源。
...
已建档作者的稳定事实进入 `data/authors.json`。普通作者的基础核验结果保留在论文级姓名、机构和关系中；检索过程与未采纳的信息只在维护阶段保留，不进入论文笔记或公开页面。
