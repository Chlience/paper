...
- 存在 `主要启发` 时，先形成一条与首要贡献对齐的首要启发；辅助启发只保留独立的认识更新。每条都完成可归因的原有判断、论文专属的区分性证据、作用机制、抽象关系、迁移后的可证伪预测和失效边界；通用检查项和复验清单已经回流到对应功能章节。
...
`主要启发` 记录材料带来的稳定认识更新。它回答三个问题：原有判断在哪些条件下不够准确，材料通过什么机制改变了该判断，以及新判断可以怎样迁移和验证。默认先构建一条与首要贡献对齐的首要启发；辅助启发只有在具备独立的原有判断、作用机制、区分性证据和迁移结果时增加，不设置目标数量。
...
新契约自 2026-08-05 10:40 起适用于新建或实质更新的 v2.1 笔记。自动检查只验证编号标题、自然段和直接证据定位；认识变化、机制、迁移预测、来源归因与失效边界继续由人工语义门禁判断。更早的存量笔记在被触及时迁移，不执行一次性全库重写。
