...
4. **贡献、方法与证据分析**：先给出贡献全景，再按实际执行顺序或理论论证的依赖顺序解释首要贡献的 `what / how / why / evidence / boundary`，说明各环节的输入、作用、传递对象和输出；抽象机制在有助于理解时加入可与原定义逐项对应的最小具体例子，为关键结果添加直接证据定位和最窄结论，把准入的高价值图放在首次解释对应机制或结果的位置。
...
- `论文脉络` 清楚区分首要贡献与辅助贡献，并按执行或论证顺序完整介绍方法；抽象机制在需要时配有可与原定义对应的具体例子。
...
方法部分按实际执行顺序或理论依赖顺序展开。至少说明输入或初始条件、关键阶段、阶段间传递的对象、每个环节解决的局部问题、输出或训练信号、设计理由，以及失败条件或成立边界。存在多项贡献时，先完整解释首要贡献，再说明辅助贡献如何支撑它。
...
2. 读者能否区分首要贡献与辅助贡献，并按方法执行或论证依赖顺序复述机制、各环节作用和结论链；抽象机制是否在需要时得到可与原定义对应的具体例子。
3. 关键结论是否绑定最直接的证据位置。
