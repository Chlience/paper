...
- 停止条件：每项核心结论都有证据定位，结论范围没有超出证据覆盖范围；首要贡献已经获得比辅助内容更深入的机制、证据与边界分析，读者仅依赖笔记即可回答它解决什么问题、增加了什么、如何成立以及证据支持到哪里。
...
6. 先做核心贡献分级：结合标题、摘要、贡献声明、方法篇幅、关键消融和后续影响，确定一项首要贡献及必要的辅助贡献。`一句话结论`、索引核心信号和正文最深入的小节应指向同一个首要贡献；若三者不一致，继续核对论文并修正优先级。
7. 按重要性分配分析深度：首要贡献需要回到定义它的原始章节、公式、算法、图、附录或代码，选择最适合该材料的表达方式完成解释，例如公式推导、伪代码、具体例子、流程图、状态转移或实现路径。辅助内容用于支撑主线，避免实验口径、背景信息或周边工程细节占据更高的解释权重。
8. 主动深化核心内容：读者若只能复述方法名称和结果，仍无法说明内部机制、关键假设、相对已有工作的变化或主要失败边界，分析尚未完成。不同材料需要的展开形式可以不同，验收标准统一为能够回答 `what / how / why / evidence / boundary`。

一句话要求：首要贡献需要从术语和效果继续展开到工作机制或论证链；涉及效率收益或关键数学变换时，同时写出朴素计算路径、优化后的计算路径、成立条件和记号约定。表达形式由材料决定，目标是让核心贡献获得与其重要性相称的解释深度。

核心内容深度审计从 `一句话结论` 和索引核心信号反向进行：正文是否有专门小节解释同一贡献，是否使用原始材料中最直接的定义与证据，是否回答 `what / how / why / evidence / boundary`，局限是否覆盖它的主要成立条件。现有工作流脚本负责结构、链接和公式语法，无法判断分析重点与深度是否合理，因此该项必须由阅读者主动检查。
...
- 是否明确一项首要贡献及必要的辅助贡献；`一句话结论`、索引核心信号和正文最深入的小节是否指向同一重点；首要贡献是否回答 `what / how / why / evidence / boundary`，并获得比周边实验口径、背景和辅助工程细节更充分的分析。
