...
- `content/utility/papers-index.md` 已更新；`当前收录` 包含唯一的简称、首次公开月份和索引核心信号，作者、机构、主题、引用或方法关系已连接到现有档案。
...
2. **时间**：使用论文或材料首次公开月份，格式固定为 `YYYY年M月`；归档时间不进入这一列。
...
4. `作者关系图谱` 按需新增 cluster 或更新已有 cluster。
5. `跨论文关系` 写入与已有论文的主题、作者、机构、引用或方法关系；主题延展需要说明新增视角。
...
2. 搜索该 slug，删除或修订其它论文、`作者关系图谱` 和 `跨论文关系` 中已经失效的关系描述。
...
- `content/utility/papers-index.md` 的 `当前收录` 是否与论文文件集合双向一致，且每行包含简称、首次公开月份和索引核心信号。
