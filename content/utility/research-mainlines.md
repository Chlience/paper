...
Updated-At: 2026-07-17 17:05

本页基于 2026-07-16 的 102 条归档材料快照，提炼八条持续演进的研究主线。它和[论文索引](/archive/)承担不同职责：索引持续提供逐篇入口，本页保留该快照下跨论文问题如何变化、哪些证据较强、哪些解释仍受反例约束。后续材料等待主线规则更新后再纳入新快照。

## 阅读方法

主线由单篇笔记中的研究问题、机制、关键实验或定理、证据强度、局限和跨论文关系共同形成。快照中的每篇论文只分配一条主线作为主要归属，并可作为其它主线的桥接、转折、证据或反例。分类服务于阅读，不表示论文只能回答一个问题。
