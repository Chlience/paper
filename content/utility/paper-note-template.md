...
先概括首要贡献与辅助贡献，并说明辅助贡献怎样支撑首要贡献。随后按实际执行顺序或理论论证的依赖顺序拆解关键环节，无需沿用论文目录。每个环节都回答：它接收什么输入或初始条件、处理什么局部问题、向下一阶段传递什么对象、产生什么输出或训练信号、为什么这样设计、在哪些条件下可能失效。

涉及效率或数学变换时，写清朴素路径、优化路径、成立条件、原始障碍和实现边界；同一段推导保持一致的 tensor / matrix 记号。抽象机制在有助于理解时加入最小具体例子；例子中的对象、操作和结果应与原定义逐项对应，并在例子后回到公式、算法或正式机制。
