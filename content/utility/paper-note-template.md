...
先概括首要贡献与辅助贡献，并说明辅助贡献怎样支撑首要贡献。方法包含多个关键阶段、角色、模型变体或训练分支时，先用一段话、列表、表格或小型流程图给出端到端概览，并区分容易混淆的同名对象。随后按实际执行顺序或理论论证的依赖顺序拆解关键环节，无需沿用论文目录。

每个关键环节都回答：

1. 它接收什么输入或初始条件；
2. 执行什么核心操作，解决什么局部问题；
3. 向下一阶段传递什么数据、表示、状态、策略或证明结果；
4. 产生什么输出、训练信号或结论；
5. 为什么采用这一设计；
6. 直接证据位于哪里；
7. 在哪些条件下成立、可能失效，以及哪些关键细节仍未披露。

方法详细度以读者能够复述完整执行链或论证链为标准，不设统一字数。阶段列表、模块名称、表格和流程图只承担导航作用，正文继续解释阶段间的接口、因果关系和信号方向。
...
继续使用与第 5 节同级的标题拆解后续关键阶段、系统路径或证明依赖；按实际内容增加第 7、8 节等章节，省略只用于包裹这些章节的“核心贡献与方法逻辑”标题。方法本身只有一个实质章节时，可以改用单个 `### 5. 方法…`。结论链条使用方法末节之后的下一个整数。
...
材料或后续交流形成会改变问题理解、系统设计或诊断方式的稳定认识更新时添加。通常选择二至四条，以证据实际支持的数量为准。

### 1. <可独立成立的认识更新>

每条以完整判断命名，正文用自然段串联证据与机制、来源归因、迁移与边界，不把这些提示写成固定字段。先定位论文直接支持的现象、对照或个案，再解释机制及其能支持的最窄判断；随后区分论文事实、作者解释、用户判断、本地分析和待验证外推，并说明适用对象、失效条件与能够区分替代解释的验证。

通用评测要求、未披露项和复验清单分别移入关键实验、局限或代码与复现章节。移除论文专属证据后仍完整成立的通用提醒不进入本节。
-->
