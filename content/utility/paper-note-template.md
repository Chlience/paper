...
`archive-core` 保留直接支撑或限制一句话结论所需的结果，数量由独立证据类型和结论边界决定；同一证据只在一个位置完整展开。删除无关字段，避免用空表格代替分析。
...
材料或后续交流形成会改变问题理解、系统设计或诊断方式的稳定认识更新时添加。首要启发与辅助启发使用同一筛选条件：候选需要具备可归因的认识变化、独立的作用机制、区分性证据和迁移预测；没有独立认识更新时省略本节，不预设数量上限或目标数量。
